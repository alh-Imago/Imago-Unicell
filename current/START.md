...
3. **A compiler path from higher-level cell/core description down to
   ...
   - **A genuinely long-range thread, captured not started
     ...
     route," now clarified (`#352`): lower the compiler's own output
     past ICM v3 configuration to real, synthesizable Verilog generated
     per-program -- a natural extension of the already-proven "many
     frontends, one shared IR" architecture, applied to the backend
     side instead. Deliberately deferred, per Alan's own words: "sort
     after all this is sorted." Not scoped for real work yet.

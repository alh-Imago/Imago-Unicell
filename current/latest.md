...
**#838 -- newest, a real prospect with legs but not finalised: an ICM-aware assembler extension.** The assembler would read an ICM's own core-type/placement content directly and auto-derive a collapsed, single-purpose FPGA build (`core_select` baked in as a compile-time constant per cell) instead of the manual `single_core` mode used today. Builds on two real existing pieces -- `project_assemble_v1.py`'s own `single_core` mode, and the never-built `icm_to_verilog_v1.py` scoped in `docs/stripped-cell/design-notes/icm_to_verilog_generator.md`. A real premise correction made along the way: the CURRENT ICM v3 format does NOT carry card/target details (that's the separate MAN file) -- the old, archived v2 format did. Genuinely open: what a "mirror generated file, checked on load" safety mechanism looks like, needed because baking `core_select` in as a constant means a mismatched runtime config would misbehave silently rather than failing loudly. Not started -- needs that mechanism designed, and a real ALM/Fmax measurement of whether compile-time constants actually collapse under Quartus (and separately Yosys) before it moves past prospect status.

**#837 -- a real, measured demo: the old postcode compare-and-swap re-run on the current substrate.** Found in the archive (`old_demo_algorithms.onion`), read directly rather than trusted from memory (the real dataset is ~1,000 postcodes, not the 1.2M recalled, and the original sorts N postcodes by distance from one point rather than comparing two directly). MEASURED on the real VM: even with zero sharing between two independent compiles, ~6x fewer cells than the old system's 711/comparator figure -- a categorical difference (whole-word ops vs. one gate per cell), not a tuning win. `nano/postcode_cas_demo_v1.py` is a real, working, geographically-correct demo. Honest limit: one compare-and-swap, not a full N-way sort network yet.

**#835/#836 -- a new prospect, paired cells reusing the existing command/programming bus for rare long-distance data transport**, bypassing ordinary cardinal routing entirely (bidirectional, data + ack both riding the bus). Alan's own honest origin for the idea: a real routing dead-end he hit and found this as the way through, not an abstract exercise. Speculative, no RTL -- real open gaps: how the time window gets arbitrated, and its relationship to the already-built (but not fully wired-through) command core.

**#829-#834 -- the photonic-interconnect thread, extended and then closed with a real numeric finding.** Real per-cell pin count for a die-edge cardinal connection, counted directly from `unicell_vix_carrier_v1.v`'s own port list, came out higher than Alan's working estimate (up to 141 bits/direction with everything counted, not just runtime data). The honest verdict: raw pin count in this range isn't unprecedented (the Arria 10 already targets a 1,152-pin package), but running that count as parallel wires with no serialization is the real problem -- the same "no shared bus" principle that drove the original stripped-cell design, pushed to a die boundary via serialization rather than reintroduced as a shared, contended bus. Alan's own closing assessment: true cell-internal photonic connectivity would be "the ultimate design" but is genuinely beyond current technology -- correctly assessed, not a discouragement. Also filed: `#830`, a real correction that "no memory model" was too strong a framing -- the memory mechanisms already exist; what's missing is the LLVM frontend mapping `load`/`store`/array syntax onto them. `#829` (documentation catch-up) remains deliberately deferred, not started.

**A real gap in session close-out discipline, caught now rather than left silent:** none of #829-#838 had been folded into this file until this pass -- two sessions' worth of ledger entries sat correctly logged in `points/points_active.md` but never cascaded up here, which is exactly the kind of drift `current/START.md`'s own close-out checklist exists to catch. Fixed now; worth watching for again.

**Separately, on its own branch (`Hex-N`), not part of this ledger's numbering:** a genuinely distinct, concept-linked spinoff project -- a hex-cell neuron design (6 faces, 16 gated in/out lines per face, threshold-and-timer firing, trained-then-frozen threshold) targeting a Tang Nano 20K with a custom-firmware BL616 link to an ESP32 front end. Explicitly not intended to merge into `main`. See `docs/hex-n/README.md` on that branch.

**Real queue, unchanged in substance from `#838`, nothing newly started:** (1) floating point -> arithmetic tiles -> bounded expression evaluator -> LaTeX, the active near-term thrust (grounded at `#827`, not yet built); (2) the LLVM/compiler gap list (`#830`); (3) scope items 3, 4, 6; (4) the VIX Carrier update backlog (`#824`); (5) `card_fit_v1` target support in `llvm_cli_v1.py`; (6) store-and-shift integration, if pursued; (7) documentation catch-up (`#829`, deferred); (8) the photonic-interconnect direction -- effectively closed/concluded by `#834`, not an open build item; (9) the paired-cell/command-bus idea (`#835`/`#836`); (10) a genuine N-way sort network building on `#837`'s proven CAS; (11) the ICM-aware collapsed-assembler idea (`#838`) -- needs the mirror/check-on-load mechanism designed and a real Quartus/Yosys measurement before it's more than a prospect.

## Previous state (as of 2026-09-22, a real documentation gap found and fixed (#828): nano/frontend_v1.py, the tabbed step-by-step front end (MAN file -> Quartus project -> Walker), was never mentioned in README.md at all. 1393 tests pass (unchanged, doc-only fix). See `points/points_active.md` #805-#828)
...
**#824 -- architectural clarification, not a correction.** Alan asked directly whether this session's DSP/BRAM/counter work (`#805`-`#822`) targets the VIX CARRIER (`unicell_vix_carrier_v1.v` -- "IX"=9, one physical position holding ALL core types simultaneously, `core_select`-switched at runtime, 128-bit config) or the older, fixed-one-core-per-position lineage (`unicell_super_v1.v`-`v8.v`, 42-bit config) this session's `SuperCell`/`SuperGrid` simulation actually runs on. Answer, confirmed against the RTL: the older lineage -- and it SHOULD have been the carrier. Alan: this does NOT invalidate the work, since "each core function still works as before, you now just have the options as you go" -- the core LOGIC (already v4c-generation, confirmed via `unicell_super_automaton_v1.py`'s own comments citing `mul_cell_v4c.v` etc.) is unaffected; only the SELECTION MECHANISM differs (build-time fixed vs runtime-configurable). A real naming collision flagged: "VIX" names TWO unrelated things in this codebase (`icm_vix_v1.py`'s hierarchical pattern format, and `vix_carrier_automaton_v1.py`'s multi-core carrier cell) -- Alan meant the second. `mul_cell_v4c.v`/`priority_cell_v4c.v` already exist as real RTL but are not yet wired into the carrier's own top-level module (9 wired today, 9-31 reserved).

**#825 -- the VM mirror into the LLVM path and a CLI, as asked.** `VMSession.from_llvm()` + `from_man(llvm=...)` (a fourth option, same uniform mechanism as dsl/python/icm_path) + `nano/llvm_cli_v1.py` (the LLVM path's first CLI, mirroring `dsl_cli_v1.py`'s shape, `--man`/`--cells` for the mirror check). **Building it surfaced a real bug:** `icm_v3.py` had NO serialization entry for `mul` or `priority` at all (added to the VM dispatch registry at `#757`/`#751`, after `icm_v3.py`'s six-core table was written, never retrofitted) -- meaning NO LLVM-compiled program using either (nearly all of them: priority for merges, mul for multiplies) could ever be saved. Fixed across THREE places (`icm_v3.py`'s own tables, `generic_field_codec_v1.py`'s separate `CORE_SELECT_TO_ROOT_KEY`, and `root_definition.json` itself), verified with a genuine round trip (compile -> save -> reload -> run -> correct result), and the existing equivalence test generalized from a hardcoded `range(6)` (which had silently never checked sequencer/branch either) to iterating every registered core.

**Earlier this session:** #805-#822 (router; fit units; fixed structures; bus width + 2D tree limit; two-port layout; counter feedback; the ~2-chain planarity limit + credit-return link; BRAM read latency; no fixed latency + ack pipeline; priority cell measured on the real VM + DSP black box; Arria 10 DSP chain topology; DSP chains placed on real sites; chain exclusivity tied into the DSP monitor; the weighted priority cell built and placed; the credit ID decoded for real; DSP-wrapper lowering via relay-hop padding). #823 filed a note on `watchdog_v1.v` as available future infrastructure.

**Real queue:** (1) store-and-shift; (2) floating point; (3) scope items 3, 4, 6; (4) the VIX Carrier update backlog (#824) -- Alan's call on priority; (5) `card_fit_v1` target support in `llvm_cli_v1.py`, if wanted.

## Previous state (as of 2026-09-22, an architectural clarification from Alan, IMPORTANT AND EASY TO MISS: this project's simulation/placement work targets the VIX CARRIER lineage, not the older per-position-fixed one -- see the boxed note below, read it before assuming otherwise. Also: the VM mirror wired into the LLVM path + a new LLVM CLI (#824), closing real queue item 8. 1361 tests pass. See `points/points_active.md` #805-#824)
...
**#815 -- Alan: DSP latency is a CARD property (leave it loose for now, maybe add to the MAN file); the Arria 10 has ~1.6k DSP units but chains cap at 27, entry/exit taps anywhere, and ONLY ONE FEED PER CHAIN no matter how much is used.** This was already anticipated at `#25`/`#26` (2026-07-09): chain length 27 is a SPINE CLOCK REGION limit (spatial, not arithmetic), and DSP latency/chain length/coordinates were already named CARD properties for the MAN file. **Built:** `device.dsp.chain` added to the Mustang MAN file (`max_length: 27`, the spine reason, `latency_per_block_cycles: null` -- deliberately unset); `card_fit_v1.dsp_chain_from_man` reads it and REFUSES a MAN file with no chain entry. `dsp_chain_v1.DspChain`: programmed once with a fixed op sequence, `.tap(entry, exit)` reads any sub-sequence without reprogramming, `.feed()` claims the WHOLE chain -- **the exclusivity rule is exact: a disjoint tap is refused just as much as an identical one, and a whole-chain feed costs the same busy state as a one-block feed.** `ChainPool` makes it operational: N chains give exactly N concurrent feeds. `ChainSegment.links(latency)` is the exact `links` list `dsp_blackbox_v1.run_dsp` (#814) takes, so the feed-in/return monitor applies to any tapped segment unchanged (verified: 8/8 correct); with no latency supplied it refuses rather than guessing (confirmed true for the real Mustang file today). `chains_available()` = `total_blocks // max_length` (62 for the Mustang), labelled an OPTIMISTIC upper bound -- the real count needs the die's spine-region geometry, not modelled.

**Mutation-checked**, one survivor investigated and found EQUIVALENT (dead code on an already-exited path), not a real gap.

**Honest:** the upper-bound chain count is not derived from real geometry; no RTL tap/entry arbiter is simulated (exclusivity is a plain mutex); not connected to `stream_layout_v1` placement or `counter_feedback_v1`; per-block latency stays unset for every real card until supplied.

**Earlier this session:** #805 router; #806 fit units; #807 fixed structures vs hardware-proven bytes; #808 bus width + 2D tree limit; #809 two-port layout; #810 counter feedback + priority cell; #811 the ~2-chain limit is K(3,n) planarity; #812 BRAM read latency + ack-driven advance; #813 no fixed latency, the ack pipeline, Alan's bus rule; #814 the priority cell measured on the real VM (weighted mode is fair, weights are 2 bits) + the DSP black box with feed-in/return monitors.

**Real queue:** (1) a real per-card DSP latency figure into a MAN file; (2) place DSP chains and taps in `stream_layout_v1` respecting the spine-region constraint; (3) tie chain exclusivity into the address-supply control; (4) the weighted priority cell into the single-port path and the layout; (5) place and decode ID -> per-chain credit; (6) host stall/refill + the empty/full signal; (7) DSP-wrapper lowering; (8) VM mirror into the LLVM path and a CLI; (9) store-and-shift; (10) floating point; (11) scope items 3, 4, 6.

## Previous state (as of 2026-09-21, the PRIORITY CELL on the single bus measured on the real VM (only WEIGHTED mode is fair; weights are 2 bits, masked silently) and the DSP chain as a BLACK BOX monitored at both ends (#814). 1259 tests pass. See `points/points_active.md` #805-#814)

## Read this first (most recent)

**#814 -- Alan: feed IN = data coming from the BRAM, feed OUT = the return values; for the single bus place a PRIORITY CELL so both chain ends keep moving at the RAM's rate; the DSP chain varies from 0/1 to over 8 per link, so BOTH the feed in and the return must be monitored or collisions occur internally where we have no control.** (1) **#813's questions resolved:** the feed-in/feed-out mapping holds; for DSP 'feed out' is the RETURN (links inside a hard cascade have no visible ack). (2) **Priority cell, MEASURED on the real VM with both ends always ready:** STRICT mode starves one end COMPLETELY (199 of 199, even at equal ranks -- fixed N>S tie-break); **WEIGHTED round-robin (`scheduling_mode=1`) is fair** -- 1:1 alternates, 3:1 gives exactly 75/25 -- so **the side thought holds provided the cell is in mode 1**. The RTL arbiter's write-priority starves the feed-in end when returns saturate. Surplus Round Robin replicated exactly and verified against the real cell for 16 configurations. **HARDWARE TRAP:** the weight field is **2 BITS and the cell MASKS larger values SILENTLY** (5 behaves as 1, 7 as 3): max ratio 3:1; the replica refuses larger. (3) **DSP black box:** no internal backpressure; two physical hazards -- a RETURN collision (result overwrites a full return side) and a FEED collision (inside the initiation interval). The monitor (feed-in count + return count, feeds only when a landing slot is guaranteed) never needs the latency, follows Little's law, and gave zero collisions across 50 random latency profiles. **Honest finding: variable latency ALONE is not the hazard** -- collisions come from a return that stalls/lags or from the initiation interval.

**Two mutation survivors handled:** D2 was a REAL BUG in my model (the feed hazard was only checked for the unmonitored feeder) -- fixed; B1 needed a hand-worked scripted test derived from the cell's SOURCE (an absent side still gains weight; a lone candidate pays only its own) -- not measured on the VM.

**Honest:** the DSP latency profile / initiation interval / hazards are modelled from Alan's description, not checked against a real cascade or the Quartus IP; the wrapper RTL holds one op at a time (this is the chain case); `counter_feedback_v1`'s single-port path still uses write priority and `stream_layout_v1` does not place a priority cell at the shared controller; replica verified on the VM only under saturation.

**Earlier this session:** #805 router; #806 fit units; #807 fixed structures vs hardware-proven bytes; #808 bus width + 2D tree limit (3/7/13); #809 two-port layout (9 chains, real BRAM sites); #810 counter feedback + priority cell; #811 the ~2-chain limit is K(3,n) planarity, one credit-return link removes it; #812 BRAM read latency + ack-driven advance; #813 no fixed latency, the ack pipeline, Alan's bus rule.

**Real queue:** (1) weighted priority cell into the single-port path of `counter_feedback_v1` and compare with write-priority; (2) place it at the shared controller in `stream_layout_v1`; (3) wire the DSP monitor to the address-supply control (return slots as credits); (4) real DSP latency/ii figures from the original doc / Quartus IP into the black box; (5) place and decode ID -> per-chain credit at the read side; (6) host stall/refill + empty/full signal; (7) address supply as a RAM cell in the layout; (8) denser router for 7, 9-13 chains; (9) DSP-wrapper lowering; (10) VM mirror into the LLVM path and a CLI; (11) store-and-shift; (12) floating point; (13) scope items 3, 4, 6.

## Previous state (as of 2026-09-21, NO FIXED LATENCY (#813): the ack is the control on the BRAM read AND on a chain of DSP links, each with its own latency; Alan's rule for which ack releases the next address recorded (mapping unconfirmed). 1220 tests pass. See `points/points_active.md` #805-#813)
...
**#812 -- Alan: in the feedback design the counter becomes a simple RAM passing data through to the BRAM controller, controlled by the results out, because the BRAM takes a few cycles; either the counter model or the feedback model needs a control mechanism.** The existing design already has it: `addr_counter_v1`'s `advance_en` is driven by the memory cell's genuine ACK, never a cycle count (#256); `bram_controller` read = one registered cycle, `cmd_valid` a one-cycle pulse; the v2 doubling to 2 cycles (#284) changed nothing elsewhere (#285). **Model (`bram_latency` 0-3, `outstanding`, `advance='ack'|'fixed'`, `addr_source='counter'|'ram'`):** ack-driven = no lost reads and correct results at every latency (rounds 11/19/27 at L=1/2/3); a counter tuned for a 1-cycle BRAM works at L=1 and SILENTLY LOSES reads at L=2 (a command into a busy interface is lost; the counter has moved on) -- the #284 doubling reproduced; holding >= latency reads in flight restores ~1 item/round; an address RAM and a counter have identical timing (the RAM handles arbitrary sequences, the counter only sequential). **The trade-off:** counter model with the ack taken from the BRAM interface alone = no chain-to-counter line, so planar for any chain count, but a stalled chain blocks the dispatch path for everyone; feedback model = a stalled chain isolated (with the priority cell), at the cost of the credit-return link (one shared link keyed by the gather stamp keeps it planar, #811). Overlapping guards found: credit and room each count reads in the BRAM and each alone is masked by the other; only removing both is caught, by a stalled-chain-with-reads-in-flight test I added after the first run showed a gap.

**Honest:** the 1-3 cycle range is Alan's recollection (RTL shows 1 and 2; not verified 3); `outstanding` depth is a parameter, not derived; protocol-level only; the placed layout still calls the address supply a 'counter' set-piece and does not model pass-through; host stall/refill and the empty/full signal not modelled.

**Earlier this session:** #805 router; #806 fit units; #807 fixed structures vs hardware-proven bytes; #808 bus width + 2D tree limit (3/7/13); #809 two-port layout (9 chains, real BRAM sites); #810 counter feedback + priority cell (grounded on the real VM cell); #811 the ~2-chain limit is K(3,n) planarity (confirmed on the router), one credit-return link removes it.

**Real queue:** (1) place and decode ID -> per-chain credit at the read side; (2) model the address supply as a RAM cell passing through to the controller in `stream_layout_v1`; (3) host stall/refill + the empty/full signal; (4) a denser router for 7, 9-13 chains; (5) the shared-port case with feedback and latency; (6) DSP-wrapper lowering + the six DSP ops; (7) wire the VM mirror into the LLVM path and a CLI; (8) store-and-shift; (9) floating point; (10) scope items 3, 4, 6.

## Previous state (as of 2026-09-21, the "probably 2 chains" limit EXPLAINED (#811): per-chain counter feedback is a K(3,n) planarity limit, confirmed on the real router; one credit-return link keyed by the gather stamp removes it (1-6 and 8 chains route). 1188 tests pass. See `points/points_active.md` #805-#811)
...
**#808 -- place the trees, test the fit (Alan), with the 40-bit caveat.** (1) **Bus width is a parameter (`BusSpec`):** a single chain needs NO routing bits so the whole word is data; a tree costs the 8-bit field as built; beats = ceil(32 / data bits); more than one beat needs a store-and-shift stage. 16-bit bus + 5 feeds -> 8 data bits, 4 beats (Alan's example); 26 -> 18 data, 2 beats. The one-chain variant, width-parameterised trees and the store-and-shift stage are PROPOSALS with no RTL. (Read "49" as 40 and "26 ... 8 bits data" as 16: stated in #808.) (2) **THE 2D LIMIT, measured exhaustively:** the routing byte rides dedicated wires between ADJACENT mux nodes, so parent and child must be unit neighbours -- and levels 1/2/3 then offer **3/7/13** leaves in 2D, not 3/9/27. The count field could address 27; geometry caps a 2D build at 13. The format reserves a third axis (6-bit slots, 4 wired), not built. The embedded 5-feed tree reproduces the hardware-proven bytes. (3) **Placement:** controller, splitter, dispatch tree, combiner tree as one rigid macro (a floor plan I chose -- an assumption) plus N replicated chains, routed by the negotiated-congestion router with each chain free to use ANY leaf pin (hand-made pairings failed); mux/combiner/splitter/controller are recorded as set-piece cells, the chains and relays are a real ICM and were RUN on the VM: 1-3 chains correct, sentinels safe, no leakage. (4) **The fit:** controller bound to a real BRAM site from the Mustang MAN; set-pieces count toward the array; honest result -- 3 tiny chains need a ~743-position array (23% used), over the ~195-position v3 budget.

**What does NOT work:** 4+ chains with this floor plan -- measured at every gap and 1-2 columns; not room, not a hard blockage; the cause is the floor plan (each chain needs BOTH trees on opposite sides, so ports interleave). Proposed U-shaped plan (trees flank the chains) not built. **A real router bug fixed:** the congestion factor overflowed the 1e18 'infinity' after ~120 iterations and produced a fake 'no path exists' -- regression-tested.

**Also this session:** #805 negotiated-congestion router; #806 fit units corrected (instantiated positions at measured ALM); #807 the fixed structures verified against hardware-proven bytes. The VM mirror exists (`vm_mirror_v1`) but is still not on the LLVM path or a CLI.

**Real queue:** (1) a floor plan that routes 4-13 chains (U-shape; needs downward-growing tree embeddings); (2) DSP-wrapper lowering + the six DSP ops in the library; (3) wire the VM mirror into the LLVM path and a CLI; (4) store-and-shift + one-chain RTL variant, if a narrow-bus card is a real target; (5) a memory model; (6) how a compile REQUESTS a stream; (7) the third axis; (8) floating point; (9) scope items 3, 4, 6.

## Previous state (as of 2026-09-21, the router rebuilt with negotiated congestion (#805), the fit model's units corrected (#806), and the BRAM/DSP fixed structures modelled and checked against hardware-proven values (#807). 1095 tests pass. See `points/points_active.md` #805-#807)
...
**2026-09-06, real core-select-via-live-programming built into the RTL
itself (#666).** `#665` found and stated plainly that the real RTL had
no mechanism for live programming to redirect `core_select` -- only
the VM's own `VixCarrierSlot` had this. This entry builds the real
counterpart directly into `unicell_vix_carrier_v1.v`: a new `awaiting_
select` register, re-armed on `program_in`'s own rising edge, cleared
the cycle the carrier consumes its first real word as a raw core-
select value. The "boot to blank" semantic reuses the carrier's own
already-proven `cfg_valid`/`cfg_data` mechanism internally rather than
inventing a new reset pathway.

**Two real, significant bugs found and fixed by testing, not assumed
away:** (1) the carrier's own `prog_ack_out` was derived only from
whichever core was currently selected -- during the select-consumption
cycle no core ever produced a real ack, leaving the sender waiting
forever. Fixed with a synthetic, direction-matched ack. (2) A genuine
priority-ordering bug: `program_in`'s first rising edge and the first
word's arrival land on the exact same cycle, and the original if-else
checked the wrong condition first, silently re-arming `awaiting_
select` forever -- every word after the first got misread as another
core-select attempt. Fixed by reordering the priority check. Neither
bug was caught by inspection; both were found because the testbenches
kept failing rather than being accepted as "close enough."

**Real, decisive verification, two testbenches:** `tb_vix_carrier_
mesh_v1.v` (`#665`'s own file, corrected -- it had been silently
passing by coincidence, since its first relayed word's low bits
happened to equal `SEL_NANO` anyway). `tb_vix_carrier_select_
redirect_v1.v` (new) -- the actual distinguishing scenario: a cell
deliberately starting on the wrong core, correctly redirected and
configured across a real cell boundary. 5/5 checks, matching the VM's
own `#658` proof exactly, now shown true in real RTL.

**Real, full regression:** all 17 Verilog testbenches pass (up from
16); the array generator re-verified at N=1, 4, 10 with the modified
carrier RTL; 557 Python tests unchanged (RTL-only work).
...
(`#665`'s own logged, deliberately-deferred finding); (4) once the
above genuinely stabilizes, return to the TRIX/cross-domain archeology
dive (`#659`'s own groundwork already done and waiting).

## Previous state (as of 2026-09-06, real carrier-to-carrier wiring in the RTL array generator -- the standing gap from #658, closed and proven in real RTL simulation, not just generated text. See `points/points_active.md` #665)

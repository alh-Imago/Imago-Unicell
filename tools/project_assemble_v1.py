...
def generate_top(top_name, n, rows, cols, cell_id_base=0x1000):
    ...
    lines = []
    lines.append(f"// {top_name}.v — points.md #552: real, generated {n}-cell array,")
    ...
    lines.append("    input  wire ENTRY_DATA,   // real, unconstrained -- guards against full-array pruning")
    ...
    lines.append("// genuine, unconstrained top-level input -- Quartus cannot prove this")
    lines.append("// constant, so it cannot prune anything this signal's own real fanout")
    lines.append("// could ever reach.")
    ...
    for idx, (r, c) in enumerate(positions):
        ...
        lines.append(f"    .cfg_valid(1'b0), .cfg_data(80'h0),")
...
def generate_sdc(man):
    return (
        "# Generated by tools/project_assemble_v1.py -- points.md #552.\n"

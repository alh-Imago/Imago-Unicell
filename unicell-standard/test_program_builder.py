...
out4 = ctrl4.run(rid4,
    inputs={info_multi.input_addresses["x"]: 1,
            info_multi.input_addresses["y"]: 1},
    capture_addresses=info_multi.output_addresses)
check("Address isolation: multi-tile build runs correctly (entry(1,1)=1)",
      out4 and out4.get(info_multi.output_addresses[0]) == 1)
...
out5, info5 = builder5.build_and_run(
    "xor_gate",
    inputs={"a": VAR_TRUE, "b": VAR_FALSE},
    controller=ctrl5,
)
check("build_and_run: XOR(1,0)=1",
      out5 and out5.get(info5.output_addresses[0]) == VAR_TRUE)
...
check("build_and_run: XOR(1,1)=0",
      out5b and out5b.get(info5b.output_addresses[0]) == VAR_FALSE)
...
for a, b, exp in nor_cases:
    ...
    got = out6.get(info6.output_addresses[0]) if out6 else None
    if got != exp:
        all_nor_ok = False
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    out7 = ctrl7.run(rid7,
        inputs={loaded_inputs["a"]: 1, loaded_inputs["b"]: 1},
        capture_addresses=loaded_outputs)
    check("Loaded image runs correctly: XNOR(1,1)=1",
          out7 and out7.get(loaded_outputs[0]) == 1)
    out7b = ctrl7.run(rid7,
        inputs={loaded_inputs["a"]: 1, loaded_inputs["b"]: 0},
        capture_addresses=loaded_outputs)
    check("Loaded image runs correctly: XNOR(1,0)=0",
          out7b and out7b.get(loaded_outputs[0]) == 0)

...
class LitePCIeUniCellSoC(SoCMini):
    def __init__(self, platform, nlanes=8):
        ...
        SoCMini.__init__(self, platform, sys_clk_freq,
            ident     = f"Imago-UniCell PCIe x{nlanes} Gen2 {NUM_CELLS} cells",
            with_ctrl = False,
        )
        ...
        self.comb += [
            led0.eq(counter[25]),              # heartbeat blink
            led1.eq(link_up),                  # PCIe link up
            led2.eq(link_up & ~counter[24]),   # activity
        ]

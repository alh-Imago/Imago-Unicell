...
STALE, NOT RUN (points.md #368): this generator's own SECTIONS list is
built almost entirely around old, mostly-archived docs (`archeology/
full-cell/docs/core/ARCHITECTURE.md`, `COMPOUND_OPCODES.md`, the Trix
ecosystem, etc.) -- running it today would regenerate `docs/manual.html`
with the SAME stale, superseded content that file was just rewritten to
remove. `docs/manual.html` was replaced directly with real, current,
appropriately-scoped content instead (NOT generated by this script,
this pass) -- rewriting THIS generator's own `SECTIONS` to point at the
current docs is real, separate, unstarted work, not attempted here.
DO NOT run this script until that rewrite happens, or it will silently
undo the current manual.html.

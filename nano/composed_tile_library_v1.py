...
def place_composed(tile: ComposedTileSpec, row: int, col: int,
                    ...
                    library: SuperTileLibrary = super_tile_library) -> List[v3.IcmV3Record]:
    """Resolve a composed tile at anchor (row, col) into real
    `IcmV3Record`s, one per sub-cell, each placed via Tier 0's own
    `place()` -- a composed tile's records are exactly what hand-placing
    each Tier-0 piece yourself would produce, just assembled from one
    call. `params` is namespaced `"{subcell_name}.{param_name}"` (e.g.
    `"cmp.threshold"`), since each sub-cell keeps its own Tier-0 param
    contract unchanged."""
    ...
    for sub in tile.subcells:
        sub_tile = library.get(sub.tile_name)

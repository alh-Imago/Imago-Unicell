# icm64_outshift.tcl — DATAPATH CONFIRM (out-shift) for the 64-bit cell on the GX660 (top_arria10_64).
# Loads a PASS_B cell with a STORED OUT-SHIFT via CMD_SET_METHOD (op 25), injects a value, and reads the FIRED
# output at probe selector 0. If out_data comes back with the RESULT right-shifted, the
# 64-bit methodology out-shift stage works on silicon.
#
#   inject 0x01002340 ; out_shift_en >>4 ; expect fired out_data = 0x00100234
#   (A/B: in-shift gives 0x10023400, out-shift gives 0x00100234 — distinct stages)
#
...
uc_open $HWM
puts "================= 64-bit datapath confirm: out-shift ================="
...
cmd 0x14A00019 0x00010800   ;# CMD_SET_METHOD (op25, auth=0xA5): out_shift_en + shift_amt=4
...
puts [format "  fired=%d  out_addr=0x%04x  out_data=0x%08x  (want 0x00100234 = 0x01002340>>4)" $seen $oa $od]
if {$seen && $od==0x00100234} {
    puts "  >>> PASS: out-shift applied on silicon — 64-bit methodology out-shift confirmed"
} elseif {$seen && $od==0x01002340} {
    puts "  >>> FAIL: fired but result NOT right-shifted — out-shift not taking on die"
} else {

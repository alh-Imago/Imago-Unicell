# icm64_shift.tcl — DATAPATH CONFIRM for the 64-bit cell on the GX660 (top_arria10_64).
# Loads a PASS_B cell with a STORED shift via CMD_SET_METHOD (op 25), injects a value,
# and reads the FIRED output at probe selector 0. If out_data comes back shifted, the
# 64-bit methodology datapath (stored shift) works on silicon.
#
#   inject 0x01002340 ; stored shift <<4 ; expect fired out_data = 0x10023400
#   (the A/B: WITHOUT the shift the same inject reads 0x01002340)
#
...
uc_open $HWM
puts "================= 64-bit datapath confirm: stored shift ================="
...
cmd 0x14A00019 0x00008800   ;# CMD_SET_METHOD (op25, auth=0xA5): in_shift_en + shift_amt=4
...
puts [format "  fired=%d  out_addr=0x%04x  out_data=0x%08x  (want 0x10023400 = 0x01002340<<4)" $seen $oa $od]
if {$seen && $od==0x10023400} {
    puts "  >>> PASS: stored shift applied on silicon — 64-bit methodology datapath confirmed"
} elseif {$seen && $od==0x01002340} {
    puts "  >>> FAIL: fired but NOT shifted — SET_METHOD/stored shift not taking on die"
} else {

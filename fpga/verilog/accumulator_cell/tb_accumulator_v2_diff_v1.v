...
//
// tb_accumulator_v2_diff_v1.v — points.md #564: real, differential
// proof that accumulator_cell_v2.v matches v1 in both modes, both
// real static and pulse-mode behavior.
`default_nettype none
`timescale 1ns / 1ps

module tb_accumulator_v2_diff_v1;
...
accumulator_cell_v2 #(.EXTERNAL_STORAGE(0)) DUT_V2_INTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
    ...
    .ack_in_n(1'b0), .ack_in_s(1'b0), .ack_in_e(ack_in_e), .ack_in_w(1'b0),
    .freeze_in(1'b0), .ready_out(), .status_negative(),
    .ext_state_in(107'h0), .ext_state_out()
);

reg [106:0] external_buffer = 107'h0;
wire [106:0] ext_next;
wire [31:0] v2ext_dout_e;
accumulator_cell_v2 #(.EXTERNAL_STORAGE(1)) DUT_V2_EXTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
...
initial begin
    ...
    @(posedge clk); #1; cfg_valid = 0;
    settle;
    check_all("after pulse-mode config");
    ...
    if (errors == 0)
        $display("PASS: %0d/%0d real checks -- accumulator_cell_v2 matches v1 in BOTH internal and external-storage modes, both static and pulse mode.", checks, checks);
    else

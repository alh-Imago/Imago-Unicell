...
//
// tb_unicell_super_v8.v — points.md #542: extends tb_unicell_super_v1.v's
...
module tb_unicell_super_v8;
    ...
    initial begin
        ...
        if (errors == 0)
            $display("PASS: unicell_super_v8 -- core selection and isolation confirmed correct across all 8 cores (compare, latch, AND accumulator now reading config continuously off core_config, no local latch, #584/#587/#592), including a substantive real branch cell test (held-reference, per-outcome routing, genuine suppression) through core_select for the first time");
        else

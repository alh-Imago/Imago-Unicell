...
//
// tb_unicell_super_v9.v — points.md #699: clones tb_unicell_super_v8.v's
// own exact real test vectors, UNMODIFIED, against unicell_super_v9.v
// -- the shell where all 4 remaining cores on the config-off-shell
// axis (RAM/adder/sequencer/branch) finally get the same real change
// compare/latch/accumulator already had (`#584`/`#587`/`#592`). Zero
// observable behavior change is exactly the claim being tested: every
// expected value below is copied verbatim from v8's own already-
// proven run, not recomputed -- if any of RAM/adder/sequencer/
// branch's own config-off-shell change had accidentally altered real
// behavior, this test would catch it directly, the same way v8 itself
// proved compare/latch/accumulator's own changes were behavior-
// preserving.
`timescale 1ns / 1ps

module tb_unicell_super_v9;
    ...
    initial begin
        ...
        if (errors == 0)
            $display("PASS: unicell_super_v9 -- core selection and isolation confirmed correct across all 8 cores, with ALL EIGHT now reading their own config continuously off core_config (RAM/adder/sequencer/branch newly fixed at #699, joining compare/latch/accumulator from #584/#587/#592; nano was always built this way) -- zero observable behavior change under normal driving, confirmed by reusing v8/v1 own exact real test vectors unmodified");
        else

...
//
// top_unicell_super_test_v6.v — points.md #584/#586: real N=1 self-
// test for unicell_super_v6.v, cloned from top_unicell_super_test_v3.v
// (real Quartus baseline 479 ALM total / 301.9 DUT ALM, #574) with
// ONLY the DUT swapped -- same real 8-core sequence, same SETTLE
// timing, same ISSP convention -- so any real ALM delta against that
// baseline is attributable to compare_cell_v3's own one real change
// (config read continuously off core_config, no local latch) alone.
`default_nettype none
`timescale 1ns / 1ps

module top_unicell_super_test_v6 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

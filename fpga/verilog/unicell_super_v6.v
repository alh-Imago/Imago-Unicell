...
//
// unicell_super_v6.v — points.md #584/#586: cloned from unicell_super_
// v3.v (real Quartus baseline 479 ALM / 301.9 DUT ALM / 107.05 MHz at
// N=1, #574; 1030.52 real avg ALM/cell at N=10, #579 -- the cheapest
// real design measured across every version tried in this project so
// far) per this project's own standing discipline (never modify a
// proven file in place). EXACTLY ONE real change from v3: the compare
// slot now instantiates `compare_cell_v3.v` (#584) instead of
// `compare_cell_v1.v`, and is wired to `core_config` (this shell's own
// STABLE, post-load register) instead of `incoming_config` (the
// transient one-shot pulse every other core here still uses).
//
// THE REAL IDEA BEING TESTED, per Alan's own real, direct proposal:
// v1's own compare core re-latches its config fields (downstream_
// mask/upstream_mask/threshold) into a private local copy on every
// cfg_valid -- a real, confirmed redundancy, since this shell's own
// `core_config` already holds that exact same information, stable,
// continuously, for as long as compare stays selected. `compare_
// cell_v3.v` removes that private copy entirely, reading its own
// config fields straight off a continuously-valid `cfg_data` input.
// Real, precise reason this is architecturally SAFE (not just
// assumed): `arrived_n/s/e/w` are already AND-gated with
// `sel_active_cmp` below, unchanged from v3 -- so compare can never
// genuinely capture a value using another core's own config bits
// (reused bit positions, shared budget) while deselected, regardless
// of what `core_config` happens to hold at that moment. See compare_
// cell_v3.v's own header for the complete real reasoning.
//
// Real, precise reason this is architecturally SAFER than v4/v5's own
// shared-storage attempts (#573/#576, both found to cost far more
// than they saved, #575/#577/#580): config is read-only from every
// core's own perspective -- a single source (the host, via cfg_valid
// into super_latch) already writes it once. There is no write-side
// arbitration needed at all, the exact mechanism #575/#580 precisely
// localized as the real, dominant cost of sharing RUNTIME state.
//
// Real, honest scope: ONLY the compare core changed here -- a
// deliberate, minimal, single-variable prototype, not a full redesign.
// If this measures out as a real, genuine saving, the same real change
// can be rolled out to the other 7 cores next; if not, it's a real,
// cheap, contained experiment to have run.
//
// unicell_super_v3.v's own real header, preserved for context:
// unicell_super_v6.v — v3, points.md #542: unicell_super_v2.v (#421/
...
module unicell_super_v6 #(
    ...
    branch_cell_v1 #(.CELL_ID(CELL_ID)) CORE_BRANCH (
        .clk(clk), .rst(rst),

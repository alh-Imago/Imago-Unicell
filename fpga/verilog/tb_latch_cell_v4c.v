// tb_latch_cell_v4c.v — points.md #720: adapted from
// tb_latch_cell_v4.v for the carrier-specific "c" variant. Confirms
// everything that variant still has (identical core behavior to v1
// including the real #295 bug fix and #522 TOGGLE priority, real
// targeted PROG_ID reconfiguration, real active=0 gating holding the
// INTERNAL latch state) -- the real addon-chain checks (invert_en) are
// REMOVED entirely, since that functionality doesn't exist at this
// core level anymore (points.md #720). The underlying latch state
// stays at 1 throughout that removed block, so nothing downstream
// depends on it.
`timescale 1ns / 1ps

module tb_latch_cell_v4c;
    ...
    // cfg_data[63:0]: [5:0]set [11:6]clear [17:12]down [23:18]toggle [63:24]reserved
    ...
    latch_cell_v4c #(.CELL_ID(16'h0003)) DUT (
    ...
    initial begin
        ...
        if (checks == 10 && errors == 0)
            $display("PASS: latch_cell_v4c -- identical core behavior to v1 including the real #295 bug fix and #522 TOGGLE priority (7 checks), real targeted PROG_ID reconfiguration surviving routing (1 check), real active=0 gating confirmed to hold the INTERNAL latch state (1 check). No addon chain at this level by design (#720).");
        else

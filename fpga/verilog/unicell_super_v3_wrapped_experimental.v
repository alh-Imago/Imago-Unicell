...
//
// unicell_super_v3.v — v3, points.md #542: unicell_super_v2.v (#421/
...
module unicell_super_v3_wrapped_experimental #(
    ...
    // ── points.md #562: EXPERIMENTAL -- the SUPER_LATCH register and
    // all per-core config-distribution logic now lives in a real,
    // separate module (super_latch_wrapper_v1.v), not inline here.
    // Zero behavioral change from unicell_super_v3.v's own original --
    // a pure structural extraction, testing whether it changes
    // Quartus's own real ALM result. ──
    wire [4:0]  core_select;
    wire [41:0] core_config;
    wire [19:0] addon_config;
    wire cfg_valid_nano, cfg_valid_ram, cfg_valid_adder, cfg_valid_acc;
    wire cfg_valid_cmp, cfg_valid_latch, cfg_valid_seq, cfg_valid_branch;
    wire sel_active_nano, sel_active_ram, sel_active_adder, sel_active_acc;
    wire sel_active_cmp, sel_active_latch, sel_active_seq, sel_active_branch;

    super_latch_wrapper_v1 CONFIG_WRAPPER (
        .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
        .core_select(core_select), .core_config(core_config), .addon_config(addon_config),
        .cfg_valid_nano(cfg_valid_nano), .cfg_valid_ram(cfg_valid_ram),
        .cfg_valid_adder(cfg_valid_adder), .cfg_valid_acc(cfg_valid_acc),
        .cfg_valid_cmp(cfg_valid_cmp), .cfg_valid_latch(cfg_valid_latch),
        .cfg_valid_seq(cfg_valid_seq), .cfg_valid_branch(cfg_valid_branch),
        .sel_active_nano(sel_active_nano), .sel_active_ram(sel_active_ram),
        .sel_active_adder(sel_active_adder), .sel_active_acc(sel_active_acc),
        .sel_active_cmp(sel_active_cmp), .sel_active_latch(sel_active_latch),
        .sel_active_seq(sel_active_seq), .sel_active_branch(sel_active_branch)
    );

    // ── incoming_config still needed directly (nano's own real cfg_data
    // remapping and the hold_in/etc ports use it pre-registration, same
    // as the original) -- NOT part of the wrapper's own output, since
    // it's the raw incoming value, not the registered one. ──
    wire [41:0] incoming_config = cfg_data[46:5];

    // ── Real, still-needed naming constants -- the output mux later in
    // this file references them by name; pure constants, not gating
    // logic, so kept here rather than exported from the wrapper. ──

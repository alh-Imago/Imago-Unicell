// tb_priority_cell_v4.v — points.md #730: the first real testbench for
// the priority-arbiter core. The one genuinely new thing to prove,
// beyond the established coverage every core testbench already has:
// when two enabled directions arrive SIMULTANEOUSLY, the real,
// configured priority rank decides which one is captured first --
// the other's own arrival is genuinely held, not lost, not merged,
// and gets captured on its own turn once the first value has been
// offered downstream.
`timescale 1ns / 1ps

module tb_priority_cell_v4;
    ...
    // cfg_data[63:0]: [5:0]up [11:6]down [13:12]rank_n [15:14]rank_s [17:16]rank_e [19:18]rank_w [39:20]addon [63:40]reserved
    ...
    priority_cell_v4 #(.CELL_ID(16'h000B)) DUT (
    ...
    initial begin
        ...
        // ── Case 4: real addon chain (invert_en). ──
        program_in = 1'b1;
        prog_send(3'd3, {1'b1, 19'h0}, 1'b1, 1'b1);   // PROG_ID_ADDON_CONFIG, invert_en
        program_in = 1'b0;
        #20;
        val_n = 32'h0000FFFF;
        pulse_n = 1'b1; #10; pulse_n = 1'b0;
        #20;
        wait (fire_e === 1'b1);
        check(out_e === ~32'h0000FFFF, "real addon chain: invert_en genuinely transforms the offered value");
        cons_ack = 1'b1; repeat (2) @(posedge clk); cons_ack = 1'b0;
        repeat (4) @(posedge clk);

        // clear addon for the final real check
        program_in = 1'b1;
        ...
        if (errors == 0)
            $display("PASS: priority_cell_v4 -- real simultaneous-arrival arbitration (higher rank wins, loser genuinely held not lost, served on its own turn), real targeted priority-order reconfiguration, real addon chain, real active=0 gating, and real weighted round-robin mode (proportional service ratio, genuine interleaving, no starvation) all confirmed");
        else

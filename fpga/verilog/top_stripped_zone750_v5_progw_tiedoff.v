...
//
// top_stripped_zone750_v5.v — points.md #195: goes further than #189/
// #191 -- rather than capping all_ready's global reach, this removes it
// entirely. #194 caught that the "read direction" #193 proposed as new
// design already existed (OP_DIAG/OP_COLLECT, #127), already wired to
// ready_bit. The real fix isn't a new mechanism, it's recognizing this
// build's live all_ready = &ready_flat (a 750-input reduction) was only
// ever needed because THIS harness wants to watch the cascade happen in
// real time during bring-up -- and that live observation can be done
// with ONE witness cell instead of aggregating all 750.
//
// FREEZE_TARGET is r=0,c=10. The opposite corner, ROW[24].COL[29], is
// the farthest cell in the grid from the freeze source -- the natural
// "last to know" witness. In every prior build (v1-v4), freeze_cascade_
// seen correctly went high once EVERY cell in the zone had gone
// not-ready; watching only the farthest cell approximates that same
// claim with a single point-to-point wire instead of a 750-input
// reduction, since in a genuine outward cascade the farthest cell is
// the last one to change -- if it goes not-ready, the cascade has
// necessarily already reached everywhere closer.
//
// This isn't a fanout/reduction problem the way the removed network
// was: ONE wire from ONE fixed cell to ONE point at the top level is
// ordinary single-net routing (like any normal long connection on any
// chip), not something that gets structurally worse as cell count
// grows -- there's nothing here for Quartus to duplicate or buffer.
`default_nettype none
`timescale 1ns / 1ps

// ── points.md #209: DIAGNOSTIC-ONLY variant, NOT functionally correct.
// prog_data_in_w/prog_arrived_in_w tied to constant 0 for every interior
// cell, matching exactly what top_stripped_grid5x5_wrapper_v1.v (#171's
// isolated 25-cell baseline) does -- deliberately breaks real west-side
// command distribution (cmd_data_row_r/cmd_arrived_row_r never reach any
// cell). Built ONLY to test whether the ~103-ALUT/cell interior cost
// #209 measured collapses back toward #171's 3.36 ALM/cell once this one
// input path is dead code again, same as the baseline. Do not flash this
// for anything other than a Resource Utilization by Entity comparison. ──
module top_stripped_zone750_v5_progw_tiedoff (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
always @(posedge clk) begin
    if (rst) begin
        ...
        case (fz_phase)
            ...
            PH_HOLD: begin
                fz_wait <= fz_wait + 16'h1;
                if (!witness_ready) freeze_cascade_seen <= 1'b1;
...
for (r = 0; r < ROWS; r = r + 1) begin : ROW
    for (c = 0; c < COLS; c = c + 1) begin : COL
        ...
        unicell_stripped_v1 #(.CELL_ID({r[4:0], c[6:0]})) CELL (
            ...
            .prog_data_in_e(32'h0), .prog_data_in_w(32'h0),   // #209 diag: tied off, matching #171's isolated baseline
            .prog_arrived_in_n(w_prog_valid[r][c]), .prog_arrived_in_s(1'b0),
            .prog_arrived_in_e(1'b0), .prog_arrived_in_w(1'b0),   // #209 diag: tied off, matching #171's isolated baseline
...
// ── points.md #195: no reduction network at all -- a single witness
// wire instead. ROW[24].COL[29] is the farthest cell from FREEZE_TARGET
// (r=0,c=10), the natural "last to know" point in a genuine outward
// cascade. Point-to-point, not a fanout/reduction -- ordinary single-net
// routing, doesn't get structurally worse as cell count grows.
wire witness_ready = c_ready[ROWS-1][COLS-1];

assign LED0_N = witness_ready;

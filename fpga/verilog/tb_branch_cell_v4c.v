// tb_branch_cell_v4c.v — points.md #720: adapted from
// tb_branch_cell_v4.v for the carrier-specific "c" variant. Confirms
// everything that variant still has (identical core behavior to v1
// including the real 3-outcome scenario, real targeted PROG_ID
// reconfiguration with the widened 4-bit ID, real active=0 gating for
// both capture paths) -- the real addon-chain check (invert_en) is
// REMOVED entirely, since that functionality doesn't exist at this
// core level anymore (points.md #720).
`timescale 1ns / 1ps

module tb_branch_cell_v4c;
    ...
    localparam [79:0] CFG_BR = {
        ...
        20'h0,              // [68:49] reserved (addon_config's own old home)
    ...
    branch_cell_v4c #(.CELL_ID(16'h0004)) DUT (
    ...
    initial begin
        ...
        if (checks == 8 && errors == 0)
            $display("PASS: branch_cell_v4c -- identical core behavior to v1 including the real 3-outcome scenario and genuine suppression (4 checks), real targeted PROG_ID reconfiguration with the widened 4-bit ID confirmed to reach route/emit fields WITHOUT releasing the held reference (2 checks), real active=0 gating confirmed for both real capture paths (2 checks). No addon chain at this level by design (#720).");
        else

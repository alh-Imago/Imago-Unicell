...
//
// unicell_super_v5.v — v5, points.md #576: cloned from unicell_super_v4.v
// (real Quartus baseline for the v4 write-mux approach: 708 ALM total,
// 539.7 for the shell+8 cores, clk_div 96.95 MHz -- #575, a real,
// direct +47.8% ALM / -9.4% Fmax COST versus v3's separate-per-core
// storage, despite genuinely cutting register count 35.7%). This file
// replaces v4's own real, precisely-localized cost driver -- an 8-way,
// 170-bit-wide value-select mux computing a whole "next" word before
// registering it -- with Alan's own real bus/chip-enable reframing:
// every core stays permanently, unconditionally wired to the SAME
// shared register (exactly like v4), but the register write is now
// PER-BIT, gated by whether the CURRENTLY SELECTED core's own real
// width actually covers that bit position. Bits outside the active
// core's own width are simply left HELD, not force-overwritten with
// zero-padding the way v4's wide case-based value did.
//
// THE REAL MECHANISM, precisely different from v4's own approach:
// - `shared_state[169:0]` -- IDENTICAL in size/purpose to v4's own
//   register (170 bits, nano's own real width, the widest of the 8).
// - Every core reads `shared_state[W-1:0]` -- UNCHANGED from v4.
// - THE REAL CHANGE: instead of computing one 170-bit `shared_state_
//   next` value via an 8-way case and registering the WHOLE word every
//   cycle, a real per-core `write_mask[169:0]` (all-ones across that
//   core's own real width, zero above) gates a per-BIT hold-vs-write:
//   `shared_state <= (shared_state & ~write_mask) | (shared_state_next
//   & write_mask)`. For any bit position only the WIDEST core (nano)
//   ever reaches, this collapses to a genuine 2-way "hold vs nano"
//   select, not an 8-way one -- matching the real, physical intuition
//   Alan named directly: cores stay wired like chips on a shared bus,
//   each with its own real chip-enable, rather than being value-muxed
//   through a single wide selector every cycle.
// - REAL, HONEST scope: `shared_state_next` itself is still built from
//   the same 8-way case as v4 (Verilog can't conditionally narrow a
//   combinational expression's own bit-width per branch) -- the real
//   bet here is that Quartus's own synthesis-time boolean optimizer
//   can prove, for any given bit position, that `shared_state_next`'s
//   contribution from a core whose own `write_mask` bit is provably 0
//   at that position is dead logic (since `& write_mask` forces it to
//   0 regardless of its value), and prune it away -- collapsing the
//   real per-bit selector width down to only the cores that genuinely
//   compete for that bit. This is a real, stated bet on Quartus's own
//   optimizer, not a guaranteed outcome -- the actual Quartus number
//   is the real next step, same as v4's own comparative build was.
// - REAL FREEZE CENTRALIZATION (`#566`) -- UNCHANGED from v4.
//
// Everything else (core_config field layout, per-core cfg_valid/
// arrived gating, the output mux, the three addons) is UNCHANGED
// from v4 -- this file only touches the write-side of the shared
// register.
//
...
module unicell_super_v5 #(
    ...
    // ── v5 REAL, ONLY CHANGE from v4: a per-CORE write_mask (all-ones
    // across that core's own real width, zero above), and a per-BIT
    // masked register write instead of an unconditional whole-word
    // one. Bits outside the active core's own real width simply HOLD
    // -- never zero-forced by a narrower core's own zero-padding the
    // way v4's wide case-based value did. This is the real, concrete
    // form of Alan's own "chip enable on a shared bus" framing: every
    // core stays permanently wired to shared_state (identical to v4),
    // but only the currently enabled core's own real bits ever get
    // written. ──
    localparam [169:0] MASK_NANO   = {170{1'b1}};
    localparam [169:0] MASK_BRANCH = {53'b0,  {117{1'b1}}};
    localparam [169:0] MASK_ACC    = {63'b0,  {107{1'b1}}};
    localparam [169:0] MASK_ADDER  = {91'b0,  {79{1'b1}}};
    localparam [169:0] MASK_CMP    = {93'b0,  {77{1'b1}}};
    localparam [169:0] MASK_SEQ    = {117'b0, {53{1'b1}}};
    localparam [169:0] MASK_RAM    = {124'b0, {46{1'b1}}};
    localparam [169:0] MASK_LATCH  = {147'b0, {23{1'b1}}};

    reg [169:0] write_mask;
    always @(*) begin
        case (write_select)
            SEL_NANO:   write_mask = MASK_NANO;
            SEL_RAM:    write_mask = MASK_RAM;
            SEL_ADDER:  write_mask = MASK_ADDER;
            SEL_ACC:    write_mask = MASK_ACC;
            SEL_CMP:    write_mask = MASK_CMP;
            SEL_LATCH:  write_mask = MASK_LATCH;
            SEL_SEQ:    write_mask = MASK_SEQ;
            SEL_BRANCH: write_mask = MASK_BRANCH;
            default:    write_mask = 170'h0;
        endcase
    end

    always @(posedge clk) begin
        if (rst) shared_state <= 170'h0;
        else shared_state <= (shared_state & ~write_mask) | (shared_state_next & write_mask);
    end

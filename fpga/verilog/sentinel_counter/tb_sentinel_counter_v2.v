// tb_sentinel_counter_v2.v — points.md #279 continuation: confirms
// sentinel_counter_v2.v's three real behaviors — normal completion
...
module tb_sentinel_counter_v2;
    ...
    sentinel_counter_v2 #(.DIFF_WIDTH(DW)) DUT (
        .clk(clk), .rst(rst),
    ...
    initial begin
        ...
        if (errors == 0)
            $display("PASS: sentinel_counter_v2 -- normal completion, diff<0 error, and diff>=2*chain_length error all confirmed correct, both error latches genuinely sticky");
        else

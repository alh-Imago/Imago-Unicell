// tb_accumulator_cell_v4c.v — points.md #720: adapted from
// tb_accumulator_cell_v4.v for the carrier-specific "c" variant.
// Confirms everything that variant still has (identical core behavior
// to v1, real targeted PROG_ID reconfiguration, real active=0 gating
// holding the INTERNAL running total) -- the real addon-chain check
// (invert_en) is REMOVED, but the real underlying accumulator advance
// that check's own inc_pulse produced is KEPT (later checks depend on
// the running total reaching 12 at that point) -- just checked as a
// plain, uninverted value now, since no addon transformation exists
// at this core level anymore (points.md #720).
`timescale 1ns / 1ps

module tb_accumulator_cell_v4c;
    ...
    // cfg_data[63:0]: [5:0]inc [11:6]dec [17:12]down [25:18]step [26]pulse [42:27]threshold [63:43]reserved
    ...
    accumulator_cell_v4c #(.CELL_ID(16'h0001), .WIDTH(32)) DUT (
    ...
    initial begin
        ...
        // points.md #720: no addon-chain programming here at all -- that
        // functionality doesn't exist at this core level anymore. The
        // real underlying accumulator advance is kept (later checks
        // depend on the running total reaching 12 here), just checked
        // as a plain, uninverted value now.
        inc_pulse = 1; #10; inc_pulse = 0; #40;
        check_now(32'd12, "inc-after-step5");
        ...
        if (checks == 7 && errors == 0)
            $display("PASS: accumulator_cell_v4c -- identical core behavior to v1 (4 checks), real targeted PROG_ID reconfiguration (step_amount, 1 check), real active=0 gating confirmed to hold the INTERNAL running total, not just the output (1 check). No addon chain at this level by design (#720).");
        else

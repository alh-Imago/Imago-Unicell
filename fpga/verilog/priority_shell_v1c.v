// priority_shell_v1c.v — points.md #730: the real cardinal wrapper
// for priority_cell_v4c.v (the carrier-specific variant, internal
// addon chain removed -- see that file's own header for the full
// reasoning). Otherwise IDENTICAL to priority_shell_v1.v.
`default_nettype none
`timescale 1ns / 1ps

module priority_shell_v1c #(
    ...
    output wire  [1:0]  status_winning_dir
);
    ...
    priority_cell_v4c #(.CELL_ID(CELL_ID)) CORE (
        ...
        .status_data_valid(status_data_valid), .status_winning_dir(status_winning_dir)
    );

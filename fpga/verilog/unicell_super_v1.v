...
//
// unicell_super_v1.v — the FIRST real "super carrier shell" / fat
...
module unicell_super_v1 #(
    ...
    // ── The FOUR real ADDONs (#311, extended #684), wired on the
    // periphery, always present regardless of which core is selected
    // (#310's own split: addons are core-independent, on the
    // periphery). Order: nibble_mask -> fine shift -> coarse
    // shift/lane -> invert (#684: fine MUST run before coarse/lane, so
    // shift_lane_addon_v2's own lane_cut math can see the true total
    // shift via shift_fine_in, not just the coarse amount alone). ──

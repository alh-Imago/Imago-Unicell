...
module top_sentinel_discrete_test_v2 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
localparam [63:0] CFG_ACC = {52'h0, DIR_E, DIR_S, DIR_N};                 // inc=N, dec=S, offer=E
...
reg cfg_done_pulse = 1'b0;   // real fix for #298's bug 3: EDGE-triggered
                              // "config genuinely just completed" pulse.
                              // The old design let S_CFGWAIT sample
                              // cfg_step==2 as a LEVEL, which is
                              // ambiguous -- cfg_step is still 2 (stale,
                              // left over from the PREVIOUS pass) for
                              // one full cycle after trigger_reconfig
                              // fires, before the reset to cfg_step<=0
                              // actually lands. S_CFGWAIT was reading
                              // that stale leftover value and falling
                              // through to S_FEED one cycle too early,
                              // landing the genuine config pulse
                              // directly on top of the first feed_pulse
                              // of the new pass -- and since
                              // accumulator_cell_v1.v's cfg_valid branch
                              // takes UNCONDITIONAL priority over its
                              // own capture logic, that first feed was
                              // silently discarded every single pass
                              // (confirmed via tb_debug_298.v: the
                              // collision is deterministic on EVERY
                              // pass after pass 0, not just the
                              // intermittent single case #298 reported
                              // -- a genuine correction to that entry,
                              // logged as #306). cfg_done_pulse fires
                              // for exactly the one cycle immediately
                              // after cfg_step's real 1->2 transition,
                              // so S_CFGWAIT can never mistake old
                              // state for new.
always @(posedge clk) begin
    ...
    if (rst) begin
        ...
        cfg_step <= 3'd0;   // re-arm the config pulse every pass, not
                             // just once at power-on (#298's bug 2 fix,
                             // unchanged)
        ...
        cfg_pulse      <= 1'b0;
        cfg_step       <= 3'd2;
        cfg_done_pulse <= 1'b1;   // genuinely just finished, this cycle only
    end
end
...
reg [3:0]  collect_target = 4'd3;   // real fix, second bug found (2026-08-14,
                                     // logged as #307): a FIXED collect
                                     // count of 3 only lands below the
                                     // comparator's threshold=8 for
                                     // feed_target=9/10 (post-collect
                                     // 6/7). Once #298's own varying-
                                     // feed_target fix let feed_target
                                     // reach 11/12, post-collect landed
                                     // AT/ABOVE threshold (8/9) -- the
                                     // comparator was then CORRECTLY
                                     // reporting "still >= threshold",
                                     // and its continuous re-firing
                                     // re-SET the latch moments after
                                     // S_UNFREEZE's clear pulse passed.
                                     // Every cell was behaving exactly
                                     // as configured -- not a cell bug,
                                     // a test-stimulus bug. Fix: collect
                                     // enough to always land at a fixed
                                     // safe margin (acc=5, comfortably
                                     // under threshold=8) regardless of
                                     // feed_target.
...
always @(posedge clk) begin
    ...
    if (rst) begin
        ...
        case (state)
            S_CFGWAIT: if (cfg_done_pulse) begin
            ...
            S_CHECK1: begin
                ...
                collect_count  <= 0;
                collect_target <= feed_target - 4'd5;   // always land at
                                                          // acc=5, safe
                                                          // margin below
                                                          // threshold=8
                state <= S_COLLECT;
            ...
            S_COLLECT: begin
                if (collect_count < collect_target) begin
                    collect_pulse <= 1'b1;

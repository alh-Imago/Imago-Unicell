// tb_nano_bounded_loop_ring_shells_v1.v — #639's own standing next
// step: rewires #638's real 4-cell bounded loop ring through the new
// real cardinal control shells (nano_shell_v1/adder_shell_v1/
// ram_shell_v1), proving real cardinal control genuinely composes
// across a real multi-cell topology, not just a single wrapped cell in
// isolation (#639's own test only ever exercised one shell at a time).
//
// Real, deliberate choice, matching #636's own established "testbench
// stands in for a not-yet-built real source" precedent: since no
// command core exists yet (#639's own explicit standing gap), this
// testbench continues to supply active/freeze/hold/reemit/update --
// but now genuinely THROUGH each shell's own real cardinal ports
// (north, consistently, across every shell here) instead of the old
// flat ports #638 used directly. This is real progress specifically
// because a control line asserted from ONE real direction is
// genuinely sufficient (proven per-shell in #639) -- the same real
// OR-combine mechanism, now exercised inside an actual working loop
// rather than a single isolated cell.
//
// Real, unchanged from #638: same 4-cell ring topology, same
// dynamic-routing loop-exit decision at LOOP_CTRL, same real bugs'
// fixes already baked into the sequencing below (reemit-after-consume
// ordering, waiting on rr_data_valid rather than adder_data_valid to
// confirm the real relay->LOOPVAR handoff actually completed). LOOP_
// CTRL is NOT wrapped in a shell here -- #639 only built shells for
// nano_gate_v4/adder_cell_v4/ram_cell_v4's OWN control-signal shapes;
// LOOP_CTRL, being a plain nano_gate_v4 instance with dynamic routing
// enabled, uses nano_shell_v1 too, same as LOOPVAR.
`timescale 1ns / 1ps

module tb_nano_bounded_loop_ring_shells_v1;
    ...
    nano_shell_v1 #(.CELL_ID(16'h7000), .ENABLE_DYNAMIC_ROUTING(1'b0)) LOOPVAR (
        .clk(clk), .rst(rst),
        ...
        .hold_in_n(hold), .hold_in_s(1'b0), .hold_in_e(1'b0), .hold_in_w(1'b0),
        .fb_internal_in_n(1'b0), .fb_internal_in_s(1'b0), .fb_internal_in_e(1'b0), .fb_internal_in_w(1'b0),
        .a_reemit_in_n(reemit), .a_reemit_in_s(1'b0), .a_reemit_in_e(1'b0), .a_reemit_in_w(1'b0),
        .a_update_in_n(upd), .a_update_in_s(1'b0), .a_update_in_e(1'b0), .a_update_in_w(1'b0),
    ...
    nano_shell_v1 #(.CELL_ID(16'h7001), .ENABLE_DYNAMIC_ROUTING(1'b1)) LOOP_CTRL (
        .clk(clk), .rst(rst),
        .active_in_n(1'b1), .active_in_s(1'b0), .active_in_e(1'b0), .active_in_w(1'b0),
        .freeze_in_n(1'b0), .freeze_in_s(1'b0), .freeze_in_e(1'b0), .freeze_in_w(1'b0),
    ...
    adder_shell_v1 #(.CELL_ID(16'h7002)) ADDER (
        .clk(clk), .rst(rst),
        .active_in_n(1'b1), .active_in_s(1'b0), .active_in_e(1'b0), .active_in_w(1'b0),
        .freeze_in_n(1'b0), .freeze_in_s(1'b0), .freeze_in_e(1'b0), .freeze_in_w(1'b0),
    ...
    ram_shell_v1 #(.CELL_ID(16'h7003)) RAM_RELAY (
        .clk(clk), .rst(rst),
        .active_in_n(1'b1), .active_in_s(1'b0), .active_in_e(1'b0), .active_in_w(1'b0),
        .freeze_in_n(1'b0), .freeze_in_s(1'b0), .freeze_in_e(1'b0), .freeze_in_w(1'b0),
    ...
    task do_round(input [31:0] bound_n, output continued, output [31:0] exit_val);
        begin
            ...
            if (lc2exit_fire) begin
                ...
                while (LOOP_CTRL.CORE.a_arrived !== 1'b1) @(posedge clk);
                #1;
            end
        end
    endtask
    ...
    initial begin
        ...
        $dumpfile("/tmp/tb_nano_bounded_loop_ring_shells_v1.vcd");
        $dumpvars(0, tb_nano_bounded_loop_ring_shells_v1);
        ...
        check(LOOPVAR.CORE.data_reg === 32'd0, "entry-seed: LOOPVAR.CORE.data_reg=0");
        ...
        check(LOOPVAR.CORE.data_reg === 32'd1, "round 1: LOOPVAR.CORE.data_reg=1 via real 4-cell ring");

        do_round(32'd3, cont, final_val);
        check(cont === 1'b1, "round 2: continued (1<3)");
        check(LOOPVAR.CORE.data_reg === 32'd2, "round 2: LOOPVAR.CORE.data_reg=2 via real 4-cell ring");

        do_round(32'd3, cont, final_val);
        check(cont === 1'b1, "round 3: continued (2<3)");
        check(LOOPVAR.CORE.data_reg === 32'd3, "round 3: LOOPVAR.CORE.data_reg=3 via real 4-cell ring");
        ...
        if (checks == 9 && errors == 0)
            $display("PASS: real bounded loop, now rewired through the real cardinal control shells (nano_shell_v1/adder_shell_v1/ram_shell_v1) -- same 4-cell physical ring as #638, active/hold/reemit/update genuinely routed through each shell's own real cardinal ports (north, single-direction, per #639's own proven-sufficient OR-combine), 3 real continue rounds (0->1->2->3), then a real exit at i==N==3");
        else

...
//
// adder_shell_v1.v — same real pattern as `nano_shell_v1.v`: a real
// cardinal wrapper, not a core edit. `adder_cell_v4.v` itself is
...
// neighbor asserting this line activates it" rule `nano_shell_v1.v`
// uses. See that file's own header for the full real design rationale
// (points.md #638's own standing queue item 1, command core
// prerequisite, per Alan's own direct design call).
`default_nettype none
`timescale 1ns / 1ps

module adder_shell_v1 #(
    ...
    adder_cell_v4 #(.CELL_ID(CELL_ID)) CORE (

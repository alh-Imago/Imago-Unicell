...
//
// top_stripped_grid5x5_both_v2.v — STEP 4 of the points.md #103 campaign,
// REBUILT (#139) combining step 2's wrapper (#127/#135) and step 3's
// corrected single-hop command-cell mechanism (#126/#138) on the SAME
// 25-cell grid, replacing the deprecated step-4 attempt (#112/#113, built
// on the now-superseded relay-chain cardinal design).
//
// Wrapper injects via North (initial setup, one-shot, per step 2).
// Command-cell companions inject via West (ongoing reprogram, per step 3).
// Genuinely separate cardinal directions -- no data conflict. program_in
// itself is a single, general control bit (#123's own reasoning), so
// where both COULD assert it, this build ORs them -- safe here because
// the wrapper's one-shot setup finishes long before the command
// mechanism's first (much slower) trigger, confirmed by construction, not
// assumed. program_done fans out to both consumers with no conflict
// (an output being read by two things is fine).
`default_nettype none
`timescale 1ns / 1ps

module top_stripped_grid5x5_both_v2 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
// ── Wrapper program driver (identical to step 2, #134's counter fix) ──
reg [4:0] prog_addr = 5'h0;
reg [2:0] prog_row  = 3'h0;
reg [2:0] prog_col  = 3'h0;
...
// ── Per-cell signal arrays — identical grid interconnect throughout ──
...
for (r = 0; r < 5; r = r + 1) begin : ROW
    for (c = 0; c < 5; c = c + 1) begin : COL
        ...
        // ── Command-cell companion, targeting THIS SAME cell (per step 3) ──
        cell_command_v1 CMD (
            .clk(clk), .rst(rst),
        ...
        unicell_stripped_v1 #(.CELL_ID({8'h0, r[3:0], c[3:0]})) CELL (
            ...
            // program_in: OR of both sources -- safe here since the
            // wrapper's one-shot setup finishes long before the command
            // mechanism's first (much slower) trigger.
            .program_in(w_program_out[r][c] | cmd_program_out[r][c]),
            .program_done(c_program_done[r][c]),

            // Genuinely separate cardinal directions for each source's
            // data -- wrapper on North (step 2), command mechanism on
            // West (step 3). No data conflict, confirmed by construction.
...
wire cmd_activity = cmd_program_out[0][0] ^ cmd_program_out[3][2] ^ cmd_program_out[4][4];

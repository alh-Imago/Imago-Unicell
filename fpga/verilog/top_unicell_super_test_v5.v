...
//
// top_unicell_super_test_v5.v — points.md #576: the third leg of the
// real comparative pipeline -- same exact test sequence, same ISSP
// convention as top_unicell_super_test_v3.v (separate per-core
// storage) and top_unicell_super_test_v4.v (shared storage via a
// wide value-select mux, real Quartus cost +47.8% ALM / -9.4% Fmax
// vs v3, #575). This one targets unicell_super_v5.v -- SAME shared
// 170-bit register as v4, but written per-bit (chip-enable style,
// Alan's own real framing) rather than as one wide muxed word every
// cycle. Any real ALM/Fmax delta against v4 here is genuinely
...
module top_unicell_super_test_v5 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

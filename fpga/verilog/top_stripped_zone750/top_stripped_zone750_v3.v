...
//
// top_stripped_zone750_v3.v — points.md #186: v2 fixed rst_sr/cmd_arrived
// (confirmed: both gone from the worst-path list) but exposed two other
// pre-existing global-fanout bottlenecks at nearly identical severity,
// previously masked behind the worse problem:
//   1. cmd_word/cmd_data broadcast -- same class of problem as v2's fix,
//      deliberately left unbuffered in v2's scope. Same row-buffer
//      treatment here (750 -> 25 -> 30 fanout).
//   2. all_ready = &ready_flat -- a NEW shape, the mirror image: 750
//      signals fanning IN to one point instead of one fanning OUT to
//      750. Fixed with the same principle in reverse: a two-level
//      reduction tree (25 row-partial-ANDs computed locally, then one
//      final AND of those 25), instead of one flat 750-input reduction.
// Both additions carry the same uniform 1-cycle latency safety argument
// as v2's fix: cmd_data only changes once every 64 cycles (long hold
// relative to a 1-cycle buffer delay), and all_ready/freeze_cascade_seen
// is monitored across a 2000-cycle HOLD phase (trivially tolerant of a
// cycle or two of added latency).
`default_nettype none
`timescale 1ns / 1ps

module top_stripped_zone750_v3 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
integer cdi;
always @(posedge clk) begin
    for (cdi = 0; cdi < ROWS; cdi = cdi + 1)
        cmd_data_row_r[cdi] <= cmd_data;
end

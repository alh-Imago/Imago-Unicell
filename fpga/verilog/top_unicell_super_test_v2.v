...
//
// top_unicell_super_test_v2.v — points.md #320 made real-hardware-
// buildable. First real Quartus attempt for unicell_super_v1.v (the
// super carrier shell). A single physical cell instance cycles through
// all 6 real core configurations in sequence — RAM, adder, accumulator,
// comparator, latch, nano — checking each one's genuine, distinguishing
// result. Directly replicates the exact sequence already proven correct
// in `tb_unicell_super_v1.v` (`#320`), translated into synthesizable
// RTL rather than testbench-only constructs.
//
...
module top_unicell_super_test_v2 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
// ── SUPER_LATCH construction, matching tb_unicell_super_v1.v's own
// pack() function exactly: {13'b reserved, addon_config[19:0],
// core_config[41:0], core_select[4:0]} = 80 bits. ──
...
// v2, points.md #421/#422: SEL_SEQ config -- VALUE_0=55, VALUE_1=66,
// SEQUENCE_LEN=2 (stored length-1=1), downstream_mask=E (matching this
// testbench's own established convention of reading results on E).
localparam [79:0] CFG_SEQ   = pack(5'd6, {4'b0, 4'b0100, 2'd1, 8'd0, 8'd0, 8'd66, 8'd55});

// ── Config-load helper, same edge-triggered "genuinely just
// completed" discipline #306 already established this session, rather
// than an ambiguous level-sample. ──
reg [4:0] cfg_step = 5'd0;
reg cfg_done_pulse = 1'b0;
localparam [4:0] SETTLE = 5'd16;   // generous margin, well past this
                                     // session's own #306/#307 lessons
                                     // about tight timing races
...
always @(posedge clk) begin
    if (rst) begin
        ...
        cfg_valid <= 1'b0;   // default: one-cycle pulse, matching every
                              // other config load in this project
        arrived_n <= 1'b0;
        arrived_w <= 1'b0;
        ack_in_e  <= 1'b0;
        ...
        case (state)
            // ═══ RAM — fixed mode, purely config-driven ═══
            ...
            // ═══ ADDER — sequential A-then-B, confirmed required by
            // adder_cell_v1.v's own real two-stage protocol ═══
            ...
            // ═══ ACCUMULATOR — 3 increments, each ack'd to refresh
            // the offered snapshot (the real requirement #320 found) ═══
            ...
            // ═══ COMPARATOR — single feed, threshold=8, value=10 ═══
            ...
            // ═══ LATCH — set on N, data must carry a genuine 1 (the
            // real requirement #320 found) ═══
            ...
            // ═══ NANO — best-effort sanity only, same honest scope as
            // #320's own testbench (full two-arrival/routing protocol
            // proven at length elsewhere; this confirms the mux path
            // delivers something real, not X) ═══
            ...
            // ═══ SEQUENCER (v2, #421/#422) — the new SEL_SEQ core. No
            // arrival needed at all (this core never captures) -- it
            // offers immediately after config. Real ordering, checked
            // carefully rather than assumed: the offered value only
            // advances (NBA) the cycle an ack genuinely completes, so
            // each value must be CHECKED BEFORE its own ack, not after
            // a settle delay following it (checking after would already
            // see the NEXT value). ═══
            S_SEQ_CHECK_INITIAL: if (settle_cnt >= SETTLE) begin
                if (data_out_e !== 32'd55) err_sticky <= 1'b1;   // VALUE_0, offered immediately
            ...
            S_SEQ_CHECK2: if (settle_cnt >= SETTLE) begin
                if (data_out_e !== 32'd66) err_sticky <= 1'b1;   // VALUE_1, advanced after the ack
            ...
            S_SEQ_CHECK3: if (settle_cnt >= SETTLE) begin
                if (data_out_e !== 32'd55) err_sticky <= 1'b1;   // wrapped back to VALUE_0 correctly
                state <= S_DONE;
            end

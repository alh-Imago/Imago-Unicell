// tb_ram_cell_v4c.v — points.md #720: adapted from tb_ram_cell_v4.v
// for the carrier-specific "c" variant. Confirms everything that
// variant still has (identical core behavior to v1, real targeted
// PROG_ID reconfiguration including the split init_data LOW/HIGH
// write, real active=0 gating) -- the real addon-chain check
// (invert_en) is REMOVED entirely, not adapted, since that
// functionality doesn't exist at this core level anymore (points.md
// #720's own real reasoning: the carrier now provides it once, shared
// across whichever core is active).
`timescale 1ns / 1ps

module tb_ram_cell_v4c;
    ...
    // cfg_data[79:0]: [5:0]down [11:6]up [12]fixed [13]load_valid [45:14]init_data [79:46]reserved
    ...
    ram_cell_v4c #(.CELL_ID(16'h0000)) DUT (
    ...
    initial begin
        ...
        if (received == 5 && errors == 0)
            $display("PASS: ram_cell_v4c -- identical core behavior to v1 (3 values), real targeted PROG_ID reconfiguration surviving routing, real split init_data LOW/HIGH write correctly reconstructed, real active=0 gating confirmed. No addon chain at this level by design (#720).");
        else

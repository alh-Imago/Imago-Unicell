// accumulator_shell_v1c.v — points.md #720: the real cardinal wrapper
// for accumulator_cell_v4c.v (the carrier-specific variant, internal
// addon chain removed -- see that file's own header for the full
// reasoning). Otherwise IDENTICAL to accumulator_shell_v1.v: same
// cardinal OR-combining for active/freeze, same port list, only the
// instantiated core differs.
`default_nettype none
`timescale 1ns / 1ps

module accumulator_shell_v1c #(
    ...
    accumulator_cell_v4c #(.CELL_ID(CELL_ID), .WIDTH(WIDTH)) CORE (

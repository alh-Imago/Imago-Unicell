// tb_mul_cell_v4c.v — points.md #724: adapted from tb_mul_cell_v4.v
// for the carrier-specific "c" variant, built alongside it from the
// start. Confirms everything that variant still has (real multiply
// including low-32-bit truncation, real targeted PROG_ID
// reconfiguration, real active=0 gating) -- the real addon-chain
// check (invert_en) is REMOVED entirely, since that functionality
// doesn't exist at this core level (points.md #724).
`timescale 1ns / 1ps

module tb_mul_cell_v4c;
    ...
    // cfg_data[63:0]: [5:0]down [11:6]up [63:12]reserved
    ...
    mul_cell_v4c #(.CELL_ID(16'h000A)) DUT (
    ...
    initial begin
        ...
        // points.md #724: no addon-chain check here at all -- that
        ...
        if (received == 4 && errors == 0)
            $display("PASS: mul_cell_v4c -- real multiply including low-32-bit truncation (3 pairs), real targeted PROG_ID reconfiguration of upstream_mask (1 pair), real active=0 gating confirmed. No addon chain at this level by design (#724).");
        else

...
//
// tb_compare_v2_diff_v1.v — points.md #564: real, differential proof
// that compare_cell_v2.v matches v1 in both modes, both real outcomes.
`default_nettype none
`timescale 1ns / 1ps

module tb_compare_v2_diff_v1;
...
compare_cell_v2 #(.EXTERNAL_STORAGE(0)) DUT_V2_INTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
    ...
    .ack_in_n(1'b0), .ack_in_s(1'b0), .ack_in_e(ack_in_e), .ack_in_w(1'b0),
    .freeze_in(1'b0), .status_data_valid(),
    .ext_state_in(77'h0), .ext_state_out()
);

reg [76:0] external_buffer = 77'h0;
wire [76:0] ext_next;
wire [31:0] v2ext_dout_e;
compare_cell_v2 #(.EXTERNAL_STORAGE(1)) DUT_V2_EXTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
...
initial begin
    ...
    if (errors == 0)
        $display("PASS: %0d/%0d real checks -- compare_cell_v2 matches v1 in BOTH internal and external-storage modes.", checks, checks);
    else

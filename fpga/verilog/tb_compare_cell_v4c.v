// tb_compare_cell_v4c.v — points.md #720: adapted from
// tb_compare_cell_v4.v for the carrier-specific "c" variant. Confirms
// everything that variant still has (identical core behavior to v1,
// real targeted PROG_ID reconfiguration including the split threshold
// LOW/HIGH write, real active=0 gating) -- the real addon-chain check
// (invert_en) is REMOVED entirely, since that functionality doesn't
// exist at this core level anymore (points.md #720).
`timescale 1ns / 1ps

module tb_compare_cell_v4c;
    ...
    reg        cfg = 0;
    reg [63:0] cfg_d = 0;
    // cfg_data[63:0]: [5:0]down [11:6]up [43:12]threshold [63:44]reserved
    // threshold=8, input on N, result out E
    localparam [63:0] CFG = {20'h0, 32'sd8, DIR_N6, DIR_E6};   // top 20 bits now genuinely reserved
    ...
    compare_cell_v4c #(.CELL_ID(16'h0002)) DUT (
    ...
    initial begin
        ...
        if (received == 6 && errors == 0)
            $display("PASS: compare_cell_v4c -- identical core behavior to v1 (4 values incl. boundary + negative), real targeted PROG_ID reconfiguration (split threshold LOW/HIGH, 2 values), real active=0 gating confirmed. No addon chain at this level by design (#720).");
        else

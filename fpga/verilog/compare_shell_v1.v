...
//
// compare_shell_v1.v — same real pattern as `nano_shell_v1.v`/
// `adder_shell_v1.v`/`ram_shell_v1.v`: a real cardinal wrapper, not a
// core edit. `compare_cell_v4.v` itself is untouched. Its own two
// flat, single-bit live-control ports (`active`, `freeze_in`) become
// real 4-way cardinal port sets here, OR-combined internally. See
// `nano_shell_v1.v`'s own header for the full real design rationale
// (points.md #638's own standing queue item 1, command core
// prerequisite, per Alan's own direct design call).
`default_nettype none
`timescale 1ns / 1ps

module compare_shell_v1 #(
    ...
    compare_cell_v4 #(.CELL_ID(CELL_ID)) CORE (

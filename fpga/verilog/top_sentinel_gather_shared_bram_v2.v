...
//
// top_sentinel_gather_shared_bram_v2.v — the first real, self-contained proof that
...
module top_sentinel_gather_shared_bram_v2 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
always @(posedge clk) begin
    ...
    if (shared_read_trigger || preload_active) read_owner <= active_dir_idx;
...
always @(posedge clk) begin
    if (rst) begin
        ...
        case (state)
            ...
            // #428/#430 queue item 1: waits on round_start_pulse now
            // (collector_relay_v1's round-robin index becoming valid)
            // instead of col_program_done (the old collector's own
            // reprogramming-complete signal, which no longer exists).
            S_ROUND_WAIT_PROG: begin
                wait_cnt <= wait_cnt + 16'd1;
                if (round_start_pulse) begin
                    q_ack_in_n <= 1;
                    wait_cnt <= 0;

// tb_combiner_cell_v2.v — points.md #257/#258 continuation: confirms
// combiner_cell_v2.v's fixed round-robin scan correctly captures from
...
module tb_combiner_cell_v2;
    ...
    // directly per combiner_cell_v2.v's own design) reserved per the
    // module's field convention even though this draft doesn't route
    // wr_cmd_* through a cardinal port.
    localparam [63:0] CFG = {45'h0, 3'b000 /*is_child all 0*/, DIR_E, DIR_S, DIR_N, DIR_W};
    ...
    combiner_cell_v2 #(.CELL_ID(16'h0008), .ADDR_WIDTH(16)) DUT (
        ...
        .routing_in_n(8'h0), .routing_in_s(8'h0), .routing_in_e(8'h0), .routing_in_w(8'h0),
    ...
    initial begin
        ...
        if (errors == 0)
            $display("PASS: combiner_cell_v2 -- fixed round-robin correctly serialized 5 real captures (incl. 2 simultaneous), correct slot-ID stamping every time, dense write-address packing (no gaps despite many empty-slot skips), read-back through the SAME shared memory bit-exact");
        else

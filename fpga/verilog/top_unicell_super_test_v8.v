...
//
// top_unicell_super_test_v8.v — points.md #592: real N=1 self-test for
// unicell_super_v8.v, cloned from top_unicell_super_test_v7.v (real
// Quartus baseline 486 ALM total / 298.7 DUT ALM / 102.9 MHz, #591)
// with ONLY the DUT swapped -- so any real ALM delta against that
// baseline is attributable to accumulator_cell_v3's own one real
// change (config read continuously, no local latch) alone. This
// build's own real number is THREE real changes deep from v3 (compare,
// latch, accumulator), each individually isolated in the ledger
// (#584, #587, #592).
`default_nettype none
`timescale 1ns / 1ps

module top_unicell_super_test_v8 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

...
//
// tb_ram_v2_diff_v1.v — points.md #563: real, differential proof that
// ram_cell_v2.v's own optional external-storage capability is
// genuinely behavior-preserving in both modes, for a different real
// width (46 bits) than latch's own 23-bit proof.
...
ram_cell_v2 #(.EXTERNAL_STORAGE(0)) DUT_V2_INTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
    ...
    .ack_in_n(1'b0), .ack_in_s(1'b0), .ack_in_e(ack_in_e), .ack_in_w(1'b0),
    .freeze_in(1'b0), .status_data_valid(),
    .ext_state_in(46'h0), .ext_state_out()
);

reg [45:0] external_buffer = 46'h0;
wire [45:0] ext_next;
wire [31:0] v2ext_dout_e;
ram_cell_v2 #(.EXTERNAL_STORAGE(1)) DUT_V2_EXTERNAL (
    .clk(clk), .rst(rst), .cfg_valid(cfg_valid), .cfg_data(cfg_data),
    .data_in_n(data_in_n), .data_in_s(32'h0), .data_in_e(32'h0), .data_in_w(32'h0),
    .arrived_n(arrived_n), .arrived_s(1'b0), .arrived_e(1'b0), .arrived_w(1'b0),
...
initial begin
    ...
    // Real config: downstream=E, upstream=N, flowing mode
    cfg_valid = 1; cfg_data = {22'h0, 1'b0, 1'b0, 4'b0001, 4'b0100};
    @(posedge clk); #1; cfg_valid = 0;
    settle;
    check_all("after config");

    // Real capture
    data_in_n = 32'hDEADBEEF; arrived_n = 1; @(posedge clk); #1; arrived_n = 0;
    settle;
    check_all("after capture");
    ...
    // Real refill after drain
    data_in_n = 32'hCAFEF00D; arrived_n = 1; @(posedge clk); #1; arrived_n = 0;
    settle;
    check_all("after refill");

    if (errors == 0)
        $display("PASS: %0d/%0d real checks -- ram_cell_v2 matches v1 in BOTH internal and external-storage modes.", checks, checks);
    else

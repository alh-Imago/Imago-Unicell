// tb_nano_gate_v4c.v — points.md #720: adapted from tb_nano_gate_v4.v
// for the carrier-specific "c" variant. Confirms everything that
// variant still has (identical core behavior to v1, real hold_in mode,
// real active=0 gating for both capture and offer) -- the real
// PROG_ID_ADDON_CONFIG checks (invert_en) are REMOVED entirely, since
// that functionality doesn't exist at this core level anymore
// (points.md #720).
`timescale 1ns / 1ps

module tb_nano_gate_v4c;
    ...
    nano_gate_v4c #(.CELL_ID(16'h0006), .ENABLE_DYNAMIC_ROUTING(1'b0)) DUT (
    ...
    initial begin
        ...
        // cardinal_edge[75:70] (addon_config's own old home is reserved again)
        cfg_and_real = 128'h0;
        ...
        if (checks == 6 && errors == 0)
            $display("PASS: nano_gate_v4c -- identical core behavior to v1 (2 checks), real hold_in mode with the held value surviving across fires (2 checks), real active=0 gating confirmed for both capture and offer (1 check + 1 resume check). No addon chain at this level by design (#720).");
        else

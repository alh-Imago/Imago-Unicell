...
//
// unicell_super_v7.v — points.md #587: cloned from unicell_super_v6.v
// (real Quartus baseline 487 ALM / 298.5 DUT ALM / 106.54 MHz at N=1,
// confirming compare_cell_v3's own real -14.3% win on CORE_CMP
// specifically, 10.5 -> 9.0 ALM) per this project's own standing
// discipline. A SECOND real core gets the same treatment: the latch
// slot now instantiates `latch_cell_v3.v` (#587) instead of `latch_
// cell_v1.v`, wired to `core_config` (stable) instead of `incoming_
// config` (transient) -- latch picked as the next-smallest real core
// (8.5 ALM at N=1, #574) per Alan's own real request to "try another
// small one first."
//
// Same real idea, same real safety reasoning as compare_cell_v3.v/
// unicell_super_v6.v's own header (see there for the complete real
// argument) -- config fields (set_dir/clear_dir/downstream_mask/
// toggle_dir) no longer re-latched locally, read straight off the
// shell's own continuously-stable core_config instead. Genuine
// runtime state (latched/out_buffer/data_valid/pending_ack) UNCHANGED.
//
// Real, honest scope: TWO of 8 real cores changed so far (compare,
// latch) -- still a deliberate, incremental rollout, not a full
// redesign in one step. 6 cores remain v1, still wired to incoming_
// config, unchanged.
//
// unicell_super_v6.v's own real header, preserved for context:
// unicell_super_v3.v's own real header, preserved for context:
// unicell_super_v7.v — v3, points.md #542: unicell_super_v2.v (#421/

// tb_adder_cell_v4c.v — points.md #720: adapted from tb_adder_cell_v4.v
// for the carrier-specific "c" variant. Confirms everything that
// variant still has (identical core behavior to v1, real targeted
// PROG_ID reconfiguration, real active=0 gating) -- the three real
// addon-chain checks (invert_en/nibble_mask/shift) are REMOVED
// entirely, not adapted, since that functionality doesn't exist at
// this core level anymore (points.md #720's own real reasoning: the
// carrier now provides it once, shared across whichever core is
// active).
`timescale 1ns / 1ps

module tb_adder_cell_v4c;
    ...
    // cfg_data[63:0]: [5:0]=downstream [11:6]=upstream [12]=subtract_mode [63:13]=reserved
    ...
    initial begin
        ...
        // ── Real, targeted reprogram back to ADD -- confirms the
        // targeted channel can flip a single field back too, not just
        // forward. ──
        program_in = 1'b1;
        ...
        // points.md #720: no addon-chain checks here at all -- that
        ...
        if (received == 6 && errors == 0)
            $display("PASS: adder_cell_v4c -- identical core behavior to v1 (3 pairs), real targeted PROG_ID reconfiguration (subtract_mode flipped forward and back without disturbing routing, 2 pairs + 1 pair), real active=0 gating confirmed. No addon chain at this level by design (#720) -- the carrier provides it once, shared.");
        else

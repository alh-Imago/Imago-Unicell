...
//   [63:9]  reserved
//
// points.md #720: renamed to command_cell_v4c for naming consistency
// with the other 8 real cores in this family, each of which got a
// real "c" (carrier) variant with an internal addon chain removed
// (this core never had one -- confirmed directly, it's the one core
// whose real job is programming/routing, not data transformation, so
// there was never anything to remove here). Real, honest config-off-
// shell note, matching every other _v4c file's own reasoning: all
// real config fields here STAY registers (this cell's own live-
// programming channel needs them independently writable); the real
// fix lives at the carrier's own shell level -- feed cfg_data from
// the stable core_config, not the transient incoming_config.
// command_cell_v4.v itself is UNCHANGED, still the real, proven,
// standalone core.

`default_nettype none
`timescale 1ns / 1ps

module command_cell_v4c #(

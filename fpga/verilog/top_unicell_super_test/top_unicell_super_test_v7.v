...
//
// top_unicell_super_test_v7.v — points.md #587: real N=1 self-test for
// unicell_super_v7.v, cloned from top_unicell_super_test_v6.v (real
// Quartus baseline 487 ALM total / 298.5 DUT ALM / 106.54 MHz, #584)
// with ONLY the DUT swapped -- same real 8-core sequence -- so any
// real ALM delta against that baseline is attributable to latch_
// cell_v3's own one real change (config read continuously, no local
// latch) alone. v6's own real baseline is itself already one real
// step past the original v3 (compare_cell_v3), so this build's own
// real number is TWO real changes deep from v3 -- both individually
// isolated in the ledger (#584, #587).
`default_nettype none
`timescale 1ns / 1ps

module top_unicell_super_test_v7 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

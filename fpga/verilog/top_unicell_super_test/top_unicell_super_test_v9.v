...
//
// top_unicell_super_test_v9.v — points.md #699: real N=1 self-test for
// unicell_super_v9.v, cloned from top_unicell_super_test_v8.v with
// ONLY the DUT swapped -- so any real ALM delta against v8's own
// baseline (486 ALM total / 298.7 DUT ALM / 102.9 MHz, #591) is
// attributable to RAM/adder/sequencer/branch's own real change (config
// read continuously, no local latch, matching compare/latch/
// accumulator's own #584/#587/#592) alone. This build closes the
// config-off-shell rollout across all 8 real cores -- not an area
// optimization run (that axis was deliberately closed at #596), a
// real correctness completion instead (see unicell_super_v9.v's own
// header for the full real reasoning).
//
// NOT YET RUN THROUGH REAL QUARTUS -- sim-verified only (iverilog),
// same honest status every core on this axis has carried so far
// (#587/#592 both state "real Quartus target built, not yet run").
// A real, separate, pre-existing issue was found while preparing
// this file, flagged here rather than silently carried forward:
// `top_unicell_super_test_v8.qsf` (and, by inheritance, v6/v7's own)
// lists `unicell_super_v3.v` in its own VERILOG_FILE set, NOT
// `unicell_super_v8.v` -- the module `top_unicell_super_test_v8.v`
// actually instantiates -- and separately still lists the OLD
// `shift_lane_addon_v1.v` instead of `shift_lane_addon_v2.v`, missing
// `shift_fine_addon_v1.v` entirely (`#684`'s own real fix, `project_
// assemble_v1.py`'s own generator was corrected then, but these
// hand-authored .qsf files were not regenerated). Real, honest
// implication: none of v6/v7/v8's own .qsf files would successfully
// compile in real Quartus as they currently stand -- consistent with
// none of them ever having been run for real. `top_unicell_super_
// test_v9.qsf` (this file's own real project file) is built correct
// from the start, not inheriting either bug.
`default_nettype none
`timescale 1ns / 1ps

module top_unicell_super_test_v9 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

...
//
// top_unicell_super_test_v4.v — points.md #573: the matching v4
// (shared external-storage shell, unicell_super_v4.v) side of the
// real comparative pair -- cloned from the sibling
// top_unicell_super_test_v3.v (itself a new file this entry, the
// first full 8-core self-test top-level v3 has ever had) with ONLY
// the DUT instantiation changed. Same real 8-core sequence, same
// SETTLE timing, same ISSP probe convention -- so any real ALM/Fmax
// difference Quartus reports between the two builds is genuinely
// attributable to the shared-storage mechanism itself (#565 item 2),
// not to a difference in what's being tested.
//
...
module top_unicell_super_test_v4 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);

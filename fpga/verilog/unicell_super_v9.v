...
//
// unicell_super_v9.v — points.md #699: cloned from unicell_super_v8.v
// per this project's own standing discipline. The FOURTH real core
// change on the config-off-shell axis, and the one that closes it out
// -- RAM, adder, sequencer, and branch (the four cores that had been
// left on v1, per v8's own header) all now instantiate their own
// `_v3` variant (`ram_cell_v3.v`/`adder_cell_v3.v`/`sequencer_cell_
// v3.v`/`branch_cell_v3.v`, all `#699`), each wired to `core_config`
// (stable) instead of `incoming_config` (transient), same real change
// and same real safety reasoning as compare/latch/accumulator's own
// `#584`/`#587`/`#592`. Nano (`unicell_stripped_v1.v`) was ALREADY
// built this way from the start and needed no change at all.
//
// Real, honest scope: ALL 8 real cores in this shell now read their
// own config fields continuously off the shell's own stable `core_
// config`/`super_latch` -- no core anywhere in this shell re-latches
// a private, potentially-stale local copy of its own config. Genuine
// runtime state (data_reg/out_buffer/ref_value/etc. per core) is
// UNCHANGED everywhere -- this is purely about which register a core
// reads its OWN config fields FROM, never about what it computes.
//
// Real, deliberate motivation, distinct from `#596`'s own real ALM-
// area framing: this closes a real correctness gap, not an area
// optimization -- a core with a private, latched config copy can go
// stale after a live reprogram that doesn't happen to also re-pulse
// THAT core's own cfg_valid, a scenario more relevant now that the
// command core's own real live-reprogramming channel (`#644`) exists
// than it was when `#596` closed the broader hardware-exploration
// track (which was about ALM savings specifically, and correctly
// found those inconclusive for 2 of the 3 cores fixed at the time).
// This entry does not reopen that area question -- it finishes the
// correctness half separately.
//
// unicell_super_v8.v's own real header, preserved for context:
// unicell_super_v8.v — points.md #592: cloned from unicell_super_v7.v
// (real Quartus baseline 486 ALM / 298.7 DUT ALM / 102.9 MHz at N=1,
// confirming both compare's and latch's own real wins hold together,
// #591) per this project's own standing discipline. A THIRD real core
// gets the same treatment: the accumulator slot now instantiates
// `accumulator_cell_v3.v` (#592) instead of `accumulator_cell_v1.v`,
// wired to `core_config` (stable) instead of `incoming_config`
// (transient) -- accumulator picked per Alan's own real request for
// "a more complex, wider" core next, specifically to see whether a
// bigger real config budget (37 bits, the widest touched by this
// thread so far) produces a saving large enough to show up clearly
// above the real build-to-build noise floor `#591` found limiting the
// two smaller cores' own aggregate visibility.
//
// Same real idea, same real safety reasoning as compare_cell_v3.v/
// latch_cell_v3.v's own headers (see there for the complete real
// argument) -- config fields (inc_dir/dec_dir/downstream_mask/step_
// amount/pulse_mode/threshold) no longer re-latched locally, read
// straight off the shell's own continuously-stable core_config
// instead. Genuine runtime state (accumulator/out_buffer/data_valid/
// pulse_pending/pending_ack) UNCHANGED.
//
// Real, honest scope: THREE of 8 real cores changed so far (compare,
// latch, accumulator) -- still a deliberate, incremental rollout, not
// a full redesign in one step. 5 cores remain v1, still wired to
// incoming_
// config, unchanged.
//
// unicell_super_v6.v's own real header, preserved for context:
// unicell_super_v3.v's own real header, preserved for context:
// unicell_super_v8.v — v3, points.md #542: unicell_super_v2.v (#421/
// #422, real Quartus baseline 305 ALM / 99.57 MHz at #526) cloned per
// this project's own standing discipline (never modify a proven file
// in place) to add an 8th real core, SEL_BRANCH (`branch_cell_v1.v`),
// giving this project's own richest, most-composed-upon core (the
// cascade counter #516, multiplication #517, and division-with-
// feedback #518 were all built on it, and it was independently
// confirmed correct on real silicon standalone, #530) its own real
// physical home in the super carrier shell for the first time --
// closing the "real VM, no RTL slot" half of the asymmetry #519 first
// named (the sequencer above is the mirror-image case: real RTL slot,
// no VM dispatch, still open). Everything below this point is
// unchanged from v2 except the SEL_BRANCH additions, clearly marked
// "v3" at each site.
//
// unicell_super_v2.v's own header, preserved for context:
// unicell_super_v1.v — v2, points.md #421/#422: unicell_super_v1.v
...
//                            4=comparator 5=latch 6=sequencer
//                            7=branch (v3, #542), 8-31 reserved
...
//                            and now branch too, ALSO exactly 42 bits
//                            -- checked directly against branch_cell_
//                            v1.v's own real field map before this was
//                            built, zero bits spare), reinterpreted per
//                            core_select, not summed across all eight.
//                            Every non-nano core's own real cfg_data
//                            layout already starts at bit 0 of its own
//                            space, confirmed directly against each
//                            core's RTL before this was built — so
//                            core_config[N:0] maps onto each one's own
//                            real fields with zero reshuffling.
...
// cardinal handshake — command-cell mode (cmd_in/cmd_out) — are OUT OF
// SCOPE for this build (hold_in/fb_internal_in/a_reemit_in/a_update_in/
// a_self_update_in were closed as a real gap in v1/v2 already, #522).
// When nano is the selected core, cmd_in/cmd_out are tied to their
// safe/inactive defaults. This is a real, known limitation, not
// something hidden.
//
// ISOLATION MECHANISM: every one of the 8 cores is always physically
...
module unicell_super_v9 #(
    ...
    localparam [4:0] SEL_NANO = 5'd0, SEL_RAM = 5'd1, SEL_ADDER = 5'd2,
                      ...
                      SEL_BRANCH = 5'd7;   // v3, points.md #542: branch_cell_v1.v's
                                           // own first real RTL core_select slot
    ...
    ram_cell_v3 #(.CELL_ID(CELL_ID)) CORE_RAM (
        .clk(clk), .rst(rst),
        .cfg_valid(cfg_valid_ram), .cfg_data({22'b0, core_config[41:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    adder_cell_v3 #(.CELL_ID(CELL_ID)) CORE_ADDER (
        .clk(clk), .rst(rst),
        ...
        .cfg_valid(cfg_valid_adder), .cfg_data({55'b0, core_config[8:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    accumulator_cell_v3 #(.CELL_ID(CELL_ID)) CORE_ACC (
        .clk(clk), .rst(rst),
        // points.md #592: wired to core_config (stable), NOT
        // incoming_config (transient) -- same real change as CORE_CMP/
        // CORE_LATCH above.
        .cfg_valid(cfg_valid_acc), .cfg_data({27'b0, core_config[36:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    compare_cell_v3 #(.CELL_ID(CELL_ID)) CORE_CMP (
        .clk(clk), .rst(rst),
        // points.md #584/#586: the ONE real change from v3 -- wired to
        // core_config (the shell's own STABLE, post-load register),
        // NOT incoming_config (the transient one-shot pulse every
        // other core here still uses). compare_cell_v3.v no longer
        // needs a one-shot pulse -- it reads its own config fields
        // continuously, straight off this wire, every cycle.
        .cfg_valid(cfg_valid_cmp), .cfg_data({24'b0, core_config[39:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    latch_cell_v3 #(.CELL_ID(CELL_ID)) CORE_LATCH (
        .clk(clk), .rst(rst),
        // points.md #587: wired to core_config (stable), NOT
        // incoming_config (transient) -- same real change as CORE_CMP
        // above.
        .cfg_valid(cfg_valid_latch), .cfg_data({48'b0, core_config[15:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    sequencer_cell_v3 #(.CELL_ID(CELL_ID)) CORE_SEQ (
        .clk(clk), .rst(rst),
        .cfg_valid(cfg_valid_seq), .cfg_data({26'b0, core_config[37:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
    ...
    // ── v3, points.md #542: branch_cell_v1.v as SEL_BRANCH -- this
    // core's own real field map (#500/#504/#497) uses EXACTLY 42 of
    // the 42-bit core_config budget, zero bits spare, confirmed
    // directly against branch_cell_v1.v's own header before this was
    // built (the same "full budget, zero reshuffling" situation as
    // RAM, the only other core that uses every bit). cfg_data field
    // map matches branch_cell_v1.v's own header exactly: upstream_dir
    // [1:0], value_source_low/equal/high[4:2], fixed_value_low/equal/
    // high[25:5], emit_low/equal/high[28:26], route_low/equal/high
    // [40:29], rolling_mode[41]. ──
    ...
    branch_cell_v3 #(.CELL_ID(CELL_ID)) CORE_BRANCH (
        .clk(clk), .rst(rst),
        .cfg_valid(cfg_valid_branch), .cfg_data({22'b0, core_config[41:0]}),
        .data_in_n(data_in_n), .data_in_s(data_in_s), .data_in_e(data_in_e), .data_in_w(data_in_w),
        ...
        .ack_in_n(ack_in_n), .ack_in_s(ack_in_s), .ack_in_e(ack_in_e), .ack_in_w(ack_in_w),
        .freeze_in(freeze_in), .ready_out(), .status_data_valid()
    );
    ...
    always @(*) begin
        case (core_select)
            ...
            SEL_BRANCH: begin
                ...
                mux_ready=1'b0;   // branch_cell_v1.v's own ready_out left unconnected --
                                   // matches this file's own established convention of
                                   // only wiring ready_out for cores that already had a
                                   // real consumer for it before this session
                mux_ack_n=br_ack_n; mux_ack_s=br_ack_s; mux_ack_e=br_ack_e; mux_ack_w=br_ack_w; mux_program_done=1'b0;
            end
            default: begin  // unassigned core_select values (8-31) -- inert, not X

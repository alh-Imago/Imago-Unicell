// tb_zone750_v5_freeze.v — same proof as tb_grid5x5_both_v2_freeze.v, at the
// full 750-cell (25x30) zone scale -- Alan's actual per-zone target.
// Confirms the freeze-cascade exercise threaded into
// top_stripped_zone750_v5.v behaves correctly before this build goes to
// Quartus for re-measurement.
`timescale 1ns / 1ps

module tb_zone750_v5_freeze;
    ...
    always @(posedge CLK_100M) begin
        ...
        if (seen_freeze_cascade && DUT.fz_phase == 2'd3 /* PH_DONE */ && DUT.witness_ready && !seen_recovery) begin
            seen_recovery <= 1'b1;
            $display("[t=%0t] witness_ready recovered after release", $time);
        end
    end
    ...
    initial begin
        ...
        if (!seen_freeze_cascade)
            ...
            $display("FAIL: cascade seen but witness_ready never recovered");
        else

// tb_command_cell_v4c.v — the first real testbench for the 9th unified-
...
module tb_command_cell_v4c;
    ...
    command_cell_v4c #(.CELL_ID(16'h9000)) CMD_TRIG (
    ...
    command_cell_v4c #(.CELL_ID(16'h9001)) CMD_PROG (
        .clk(clk), .rst(rst), .active(1'b1),
    ...
    initial begin
        $dumpfile("/tmp/tb_command_cell_v4c.vcd");
        $dumpvars(0, tb_command_cell_v4c);
        ...
        if (checks == 12 && errors == 0)
            $display("PASS: command_cell_v4c -- trigger mode's genuine symmetric toggle (4/4 real cases) AND programmer mode genuinely programming a real, fresh nano_gate_v4 target end to end (3 real words, freeze-safe prog_ack pacing, toggle-pattern-recognized COMPLETE, real functional confirmation the target actually works afterward)");
        else

...
//
// top_stripped_grid5x5_cardinal_v1.v — STEP 3 of the points.md #103
// measurement campaign: the SAME 25-cell grid as #106's step-1 baseline
// (146 ALMs, 257.14 MHz), NO wrapper (#99/#108/#109) this time — instead,
// cell_cardinal_cmd_v1 (#100) attached to every cell, with command tokens
// riding the SAME N/S/E/W adjacency the data grid already uses, rather
// than a separate daisy-chain bus topology.
`default_nettype none
`timescale 1ns / 1ps

module top_stripped_grid5x5_cardinal_v1 (
    input  wire CLK_100M,
    output wire LED0_N,
    output wire LED1_N
);
...
// ── Program driver — injects PROGRAM tokens at cell (0,0)'s north cmd
// port, the SAME entry point data uses, per #100's framing. ──
...
// ── Per-cell command-channel signal arrays: SAME grid adjacency, new bus ──
...
wire chain_tail_bit = cd_n[4][4][0] ^ cv_n[4][4];

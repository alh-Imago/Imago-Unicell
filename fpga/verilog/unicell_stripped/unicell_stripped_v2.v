...
//
// unicell_stripped_v2.v — points.md #189/#190: the 256-bit unified-latch
// rebuild. Cloned from unicell_stripped_v1.v (never modify a proven file
// in place). PURELY ADDITIVE over v1's 128-bit layout, deliberately not a
// full field reshuffle: every currently-used bit position is UNCHANGED
// (topology[9:0], is_command_cell[10], ready_bit[13], routing_mask[69:64],
// cardinal_edge[75:70], pattern_low[79:76]/equal[85:82]/high[91:88],
// dynamic_route_en[94], out_buffer[127:96] all stay exactly where they
// were) -- the new [255:128] range is fresh allocation, and that's the
// ONLY real change to cmd_latch's own field map. Two decisions from
// Alan, both real corrections to an earlier draft that would have moved
// state the wrong direction, not assumed:
//   1. The wrapper's 6 persistent control-line bits (freeze_in/hold_in/
//      fb_internal_in/a_reemit_in/a_update_in/a_self_update_in) STAY on
//      cell_wrapper_v2 -- measured to help LUT count there, not moved.
//   2. Addons connect via their OWN dedicated port bundle per addon (the
//      same pattern points.md #131 already proved for the programming
//      mechanism's dedicated wires) -- ZERO latch bits reserved for
//      addon control. The cell exposes data; the addon carries its own
//      controlling mechanism externally, wired in, exactly like the
//      wrapper already does for control lines.
// Net effect: cmd_latch's only real content change is data_reg folding
// IN from a separate 32-bit reg into cmd_latch[159:128] -- "the logic
// and data store" from Alan's own framing -- with the remaining 96 bits
// ([255:160]) left genuinely unclaimed, reserved for whatever real
// future need shows up (not addon-specific, since addons take zero
// latch bits by design).
//
...
module unicell_stripped_v2 #(
    ...
    input  wire [255:0] cfg_data,   // points.md #189: widened 128->256
    ...
    // ── State ───────────────────────────────────────────────────────────
    // points.md #189: widened 128->256. [127:0] field map UNCHANGED from
    // v1 (see file header). [159:128] = data_reg, folded in from a
    // separate reg (below). [255:160] genuinely reserved, unclaimed.
    reg [255:0] cmd_latch  = 256'h0;
    // points.md #189: folded into cmd_latch[159:128] (was a separate reg).
    // Read as a wire alias everywhere below; writes now target the latch
    // slice directly (three sites, all in the single always block that
    // already owned cmd_latch's other writes -- same mutual-exclusion
    // guarantee, nothing about the write conditions changed).
    wire [31:0] data_reg = cmd_latch[159:128];   // working register — mirrors unicell64_v3.v
    ...
    always @(posedge clk) begin
        if (rst) begin
            cmd_latch     <= 256'h0;   // points.md #189: clears data_reg too, now folded in
            a_arrived     <= 1'b0;
            pending_ack   <= 6'h0;
            program_done_r<= 1'b0;
            error_frozen  <= 1'b0;
            ...
            if (programming_active) begin
                ...
                if (a_self_update_in)
                    cmd_latch[159:128] <= computed_output;   // points.md #189: was data_reg
                else
                ...
                cmd_latch[159:128] <= arrived_val;   // points.md #189: was data_reg
